...
func (r *mutationResolver) DeleteSnapshotSystemPassingAlarm(ctx context.Context, where model.SnapshotSystemPassingAlarmBoolExp) (*model.SnapshotSystemPassingAlarmMutationResponse, error) {
	...
	var m = &model1.SnapshotSystemPassingAlarm{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteSnapshotSystemPassingAlarmByPk(ctx context.Context, id int64) (*model1.SnapshotSystemPassingAlarm, error) {
	var rs model1.SnapshotSystemPassingAlarm
	m := &model1.SnapshotSystemPassingAlarm{}
	tx := db.DB.Model(m)
	// 查询记录
...
func (r *mutationResolver) DeleteSnapshotSystemPassingAlarmByUnionPk(ctx context.Context, snapshotSystemPassingAlarmID string) (*model1.SnapshotSystemPassingAlarm, error) {
	var rs model1.SnapshotSystemPassingAlarm
	m := &model1.SnapshotSystemPassingAlarm{}
	tx := db.DB.Model(&model1.SnapshotSystemPassingAlarm{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", snapshotSystemPassingAlarmID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertSnapshotSystemPassingAlarmOne(ctx context.Context, objects model.SnapshotSystemPassingAlarmInsertInput) (*model1.SnapshotSystemPassingAlarm, error) {
...
func (r *mutationResolver) UpdateSnapshotSystemPassingAlarmByUnionPk(ctx context.Context, inc *model.SnapshotSystemPassingAlarmIncInput, set *model.SnapshotSystemPassingAlarmSetInput, snapshotSystemPassingAlarmID string) (*model1.SnapshotSystemPassingAlarm, error) {
	var rs model1.SnapshotSystemPassingAlarm
	var m = &model1.SnapshotSystemPassingAlarm{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", snapshotSystemPassingAlarmID)
	qt := util.NewQueryTranslator(tx, &model1.SnapshotSystemPassingAlarm{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) SnapshotSystemPassingAlarmByUnionPk(ctx context.Context, snapshotSystemPassingAlarmID string) (*model1.SnapshotSystemPassingAlarm, error) {
	var rs model1.SnapshotSystemPassingAlarm
	var m = &model1.SnapshotSystemPassingAlarm{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := snapshotSystemPassingAlarmID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", snapshotSystemPassingAlarmID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
