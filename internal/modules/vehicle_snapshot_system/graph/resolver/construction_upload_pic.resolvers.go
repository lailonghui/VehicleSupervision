...
func (r *mutationResolver) DeleteConstructionUploadPic(ctx context.Context, where model.ConstructionUploadPicBoolExp) (*model.ConstructionUploadPicMutationResponse, error) {
	...
	var m = &model1.ConstructionUploadPic{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteConstructionUploadPicByPk(ctx context.Context, id int64) (*model1.ConstructionUploadPic, error) {
	var rs model1.ConstructionUploadPic
	m := &model1.ConstructionUploadPic{}
	tx := db.DB.Model(m)
	// 查询记录
...
func (r *mutationResolver) DeleteConstructionUploadPicByUnionPk(ctx context.Context, constructionUploadPicID string) (*model1.ConstructionUploadPic, error) {
	var rs model1.ConstructionUploadPic
	m := &model1.ConstructionUploadPic{}
	tx := db.DB.Model(&model1.ConstructionUploadPic{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", constructionUploadPicID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertConstructionUploadPicOne(ctx context.Context, objects model.ConstructionUploadPicInsertInput) (*model1.ConstructionUploadPic, error) {
...
func (r *mutationResolver) UpdateConstructionUploadPicByUnionPk(ctx context.Context, inc *model.ConstructionUploadPicIncInput, set *model.ConstructionUploadPicSetInput, constructionUploadPicID string) (*model1.ConstructionUploadPic, error) {
	var rs model1.ConstructionUploadPic
	var m = &model1.ConstructionUploadPic{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", constructionUploadPicID)
	qt := util.NewQueryTranslator(tx, &model1.ConstructionUploadPic{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) ConstructionUploadPicByUnionPk(ctx context.Context, constructionUploadPicID string) (*model1.ConstructionUploadPic, error) {
	var rs model1.ConstructionUploadPic
	var m = &model1.ConstructionUploadPic{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := constructionUploadPicID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", constructionUploadPicID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
