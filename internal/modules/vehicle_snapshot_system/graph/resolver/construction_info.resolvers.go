...
func (r *mutationResolver) DeleteConstructionInfoByUnionPk(ctx context.Context, constructionInfoID string) (*model1.ConstructionInfo, error) {
	var rs model1.ConstructionInfo
	m := &model1.ConstructionInfo{}
	tx := db.DB.Model(&model1.ConstructionInfo{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", constructionInfoID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertConstructionInfoOne(ctx context.Context, objects model.ConstructionInfoInsertInput) (*model1.ConstructionInfo, error) {
...
func (r *mutationResolver) UpdateConstructionInfoByUnionPk(ctx context.Context, inc *model.ConstructionInfoIncInput, set *model.ConstructionInfoSetInput, constructionInfoID string) (*model1.ConstructionInfo, error) {
	var rs model1.ConstructionInfo
	var m = &model1.ConstructionInfo{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", constructionInfoID)
	qt := util.NewQueryTranslator(tx, &model1.ConstructionInfo{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) ConstructionInfoByUnionPk(ctx context.Context, constructionInfoID string) (*model1.ConstructionInfo, error) {
	var rs model1.ConstructionInfo
	var m = &model1.ConstructionInfo{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := constructionInfoID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", constructionInfoID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
