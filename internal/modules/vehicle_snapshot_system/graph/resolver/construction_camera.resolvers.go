...
func (r *mutationResolver) DeleteConstructionCamera(ctx context.Context, where model.ConstructionCameraBoolExp) (*model.ConstructionCameraMutationResponse, error) {
	...
	var m = &model1.ConstructionCamera{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteConstructionCameraByUnionPk(ctx context.Context, constructionCameraID string) (*model1.ConstructionCamera, error) {
	var rs model1.ConstructionCamera
	m := &model1.ConstructionCamera{}
	tx := db.DB.Model(&model1.ConstructionCamera{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", constructionCameraID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertConstructionCameraOne(ctx context.Context, objects model.ConstructionCameraInsertInput) (*model1.ConstructionCamera, error) {
...
func (r *mutationResolver) UpdateConstructionCameraByUnionPk(ctx context.Context, inc *model.ConstructionCameraIncInput, set *model.ConstructionCameraSetInput, constructionCameraID string) (*model1.ConstructionCamera, error) {
	var rs model1.ConstructionCamera
	var m = &model1.ConstructionCamera{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", constructionCameraID)
	qt := util.NewQueryTranslator(tx, &model1.ConstructionCamera{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) ConstructionCameraByUnionPk(ctx context.Context, constructionCameraID string) (*model1.ConstructionCamera, error) {
	var rs model1.ConstructionCamera
	var m = &model1.ConstructionCamera{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := constructionCameraID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", constructionCameraID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
