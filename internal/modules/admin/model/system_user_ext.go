...
// 新建主键dataloader
func (t *SystemUserPkLoader) NewLoader(ctx context.Context) *SystemUserPkLoader {
	return &SystemUserPkLoader{
		wait:     2 * time.Millisecond,
		maxBatch: 100,
		fetch: func(keys []string) ([]*SystemUser, []error) {
			var rs []*SystemUser = make([]*SystemUser, len(keys))
			var m SystemUser
			wg := sync.WaitGroup{}
...
// 新建联合主键dataloader
func (t *SystemUserUnionPkLoader) NewLoader(ctx context.Context) *SystemUserUnionPkLoader {
