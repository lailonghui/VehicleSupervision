...
// 新建主键dataloader
func (t *EnterprisePkLoader) NewLoader(ctx context.Context) *EnterprisePkLoader {
	return &EnterprisePkLoader{
		wait:     2 * time.Millisecond,
		maxBatch: 100,
		fetch: func(keys []string) ([]*Enterprise, []error) {
			var rs []*Enterprise = make([]*Enterprise, len(keys))
			var m Enterprise
			wg := sync.WaitGroup{}
...
// 新建联合主键dataloader
func (t *EnterpriseUnionPkLoader) NewLoader(ctx context.Context) *EnterpriseUnionPkLoader {
