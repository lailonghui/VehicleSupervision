...
func (r *mutationResolver) DeleteEnterpriseByUnionPk(ctx context.Context, enterpriseID string) (*model1.Enterprise, error) {
	...
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", enterpriseID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *queryResolver) EnterpriseByUnionPk(ctx context.Context, enterpriseID string) (*model1.Enterprise, error) {
	...
	if middle.IsEnableGqlCache(ctx) {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", enterpriseID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
					_ = cacheAspect.SetNotExistUnionPkQueryCache(ctx, cacheKey, model1.Enterprise{})
				}
				return &rs, nil
			}
			return nil, err
