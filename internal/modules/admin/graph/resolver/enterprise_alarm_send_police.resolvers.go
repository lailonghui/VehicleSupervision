...
func (r *mutationResolver) DeleteEnterpriseAlarmSendPolice(ctx context.Context, where model.EnterpriseAlarmSendPoliceBoolExp) (*model.EnterpriseAlarmSendPoliceMutationResponse, error) {
	...
	var m = &model1.EnterpriseAlarmSendPolice{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteEnterpriseAlarmSendPoliceByPk(ctx context.Context, id int64) (*model1.EnterpriseAlarmSendPolice, error) {
	var rs model1.EnterpriseAlarmSendPolice
	m := &model1.EnterpriseAlarmSendPolice{}
	tx := db.DB.Model(m)
	// 查询记录
...
func (r *mutationResolver) DeleteEnterpriseAlarmSendPoliceByUnionPk(ctx context.Context, enterpriseAlarmSendPoliceID string) (*model1.EnterpriseAlarmSendPolice, error) {
	var rs model1.EnterpriseAlarmSendPolice
	m := &model1.EnterpriseAlarmSendPolice{}
	tx := db.DB.Model(&model1.EnterpriseAlarmSendPolice{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", enterpriseAlarmSendPoliceID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertEnterpriseAlarmSendPoliceOne(ctx context.Context, objects model.EnterpriseAlarmSendPoliceInsertInput) (*model1.EnterpriseAlarmSendPolice, error) {
...
func (r *mutationResolver) UpdateEnterpriseAlarmSendPoliceByUnionPk(ctx context.Context, inc *model.EnterpriseAlarmSendPoliceIncInput, set *model.EnterpriseAlarmSendPoliceSetInput, enterpriseAlarmSendPoliceID string) (*model1.EnterpriseAlarmSendPolice, error) {
	var rs model1.EnterpriseAlarmSendPolice
	var m = &model1.EnterpriseAlarmSendPolice{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", enterpriseAlarmSendPoliceID)
	qt := util.NewQueryTranslator(tx, &model1.EnterpriseAlarmSendPolice{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) EnterpriseAlarmSendPoliceByUnionPk(ctx context.Context, enterpriseAlarmSendPoliceID string) (*model1.EnterpriseAlarmSendPolice, error) {
	var rs model1.EnterpriseAlarmSendPolice
	var m = &model1.EnterpriseAlarmSendPolice{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := enterpriseAlarmSendPoliceID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", enterpriseAlarmSendPoliceID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
					_ = cacheAspect.SetNotExistUnionPkQueryCache(ctx, cacheKey, model1.EnterpriseAlarmSendPolice{})
				}
				return &rs, nil
			}
			return nil, err
