...
func (r *mutationResolver) DeleteEnterpriseUkeyByUnionPk(ctx context.Context, ukeyID string) (*model1.EnterpriseUkey, error) {
	...
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", ukeyID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *queryResolver) EnterpriseUkeyByUnionPk(ctx context.Context, ukeyID string) (*model1.EnterpriseUkey, error) {
	...
	if middle.IsEnableGqlCache(ctx) {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", ukeyID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
					_ = cacheAspect.SetNotExistUnionPkQueryCache(ctx, cacheKey, model1.EnterpriseUkey{})
				}
				return &rs, nil
			}
			return nil, err
