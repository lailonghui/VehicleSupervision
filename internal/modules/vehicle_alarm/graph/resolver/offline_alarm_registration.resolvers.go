...
func (r *mutationResolver) DeleteOfflineAlarmRegistration(ctx context.Context, where model.OfflineAlarmRegistrationBoolExp) (*model.OfflineAlarmRegistrationMutationResponse, error) {
	...
	var m = &model1.OfflineAlarmRegistration{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteOfflineAlarmRegistrationByPk(ctx context.Context, id int64) (*model1.OfflineAlarmRegistration, error) {
	var rs model1.OfflineAlarmRegistration
	m := &model1.OfflineAlarmRegistration{}
	tx := db.DB.Model(m)
	// 查询记录
...
func (r *mutationResolver) DeleteOfflineAlarmRegistrationByUnionPk(ctx context.Context, offlineAlarmRegistrationID string) (*model1.OfflineAlarmRegistration, error) {
	var rs model1.OfflineAlarmRegistration
	m := &model1.OfflineAlarmRegistration{}
	tx := db.DB.Model(&model1.OfflineAlarmRegistration{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", offlineAlarmRegistrationID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertOfflineAlarmRegistrationOne(ctx context.Context, objects model.OfflineAlarmRegistrationInsertInput) (*model1.OfflineAlarmRegistration, error) {
...
func (r *mutationResolver) UpdateOfflineAlarmRegistrationByUnionPk(ctx context.Context, inc *model.OfflineAlarmRegistrationIncInput, set *model.OfflineAlarmRegistrationSetInput, offlineAlarmRegistrationID string) (*model1.OfflineAlarmRegistration, error) {
	var rs model1.OfflineAlarmRegistration
	var m = &model1.OfflineAlarmRegistration{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", offlineAlarmRegistrationID)
	qt := util.NewQueryTranslator(tx, &model1.OfflineAlarmRegistration{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) OfflineAlarmRegistrationByUnionPk(ctx context.Context, offlineAlarmRegistrationID string) (*model1.OfflineAlarmRegistration, error) {
	var rs model1.OfflineAlarmRegistration
	var m = &model1.OfflineAlarmRegistration{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := offlineAlarmRegistrationID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", offlineAlarmRegistrationID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
