...
func (r *mutationResolver) DeleteVideoPlatformAlarmType(ctx context.Context, where model.VideoPlatformAlarmTypeBoolExp) (*model.VideoPlatformAlarmTypeMutationResponse, error) {
	...
	var m = &model1.VideoPlatformAlarmType{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteVideoPlatformAlarmTypeByPk(ctx context.Context, id int64) (*model1.VideoPlatformAlarmType, error) {
	var rs model1.VideoPlatformAlarmType
	m := &model1.VideoPlatformAlarmType{}
	tx := db.DB.Model(m)
	// 查询记录
...
func (r *mutationResolver) DeleteVideoPlatformAlarmTypeByUnionPk(ctx context.Context, videoPlatformAlarmTypeID string) (*model1.VideoPlatformAlarmType, error) {
	var rs model1.VideoPlatformAlarmType
	m := &model1.VideoPlatformAlarmType{}
	tx := db.DB.Model(&model1.VideoPlatformAlarmType{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", videoPlatformAlarmTypeID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertVideoPlatformAlarmType(ctx context.Context, objects []*model.VideoPlatformAlarmTypeInsertInput) (*model.VideoPlatformAlarmTypeMutationResponse, error) {
	rs := make([]*model1.VideoPlatformAlarmType, 0)
	m := &model1.VideoPlatformAlarmType{}
	// 结构转换
...
func (r *mutationResolver) InsertVideoPlatformAlarmTypeOne(ctx context.Context, objects model.VideoPlatformAlarmTypeInsertInput) (*model1.VideoPlatformAlarmType, error) {
...
func (r *mutationResolver) UpdateVideoPlatformAlarmTypeByUnionPk(ctx context.Context, inc *model.VideoPlatformAlarmTypeIncInput, set *model.VideoPlatformAlarmTypeSetInput, videoPlatformAlarmTypeID string) (*model1.VideoPlatformAlarmType, error) {
	var rs model1.VideoPlatformAlarmType
	var m = &model1.VideoPlatformAlarmType{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", videoPlatformAlarmTypeID)
	qt := util.NewQueryTranslator(tx, &model1.VideoPlatformAlarmType{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) VideoPlatformAlarmTypeByUnionPk(ctx context.Context, videoPlatformAlarmTypeID string) (*model1.VideoPlatformAlarmType, error) {
	var rs model1.VideoPlatformAlarmType
	var m = &model1.VideoPlatformAlarmType{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := videoPlatformAlarmTypeID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", videoPlatformAlarmTypeID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
