...
func (r *mutationResolver) DeleteDataDictionaryByUnionPk(ctx context.Context, dictionaryID string) (*model1.DataDictionary, error) {
	var rs model1.DataDictionary
	m := &model1.DataDictionary{}
	tx := db.DB.Model(&model1.DataDictionary{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", dictionaryID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertDataDictionaryOne(ctx context.Context, objects model.DataDictionaryInsertInput) (*model1.DataDictionary, error) {
...
func (r *mutationResolver) UpdateDataDictionaryByUnionPk(ctx context.Context, inc *model.DataDictionaryIncInput, set *model.DataDictionarySetInput, dictionaryID string) (*model1.DataDictionary, error) {
	var rs model1.DataDictionary
	var m = &model1.DataDictionary{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", dictionaryID)
	qt := util.NewQueryTranslator(tx, &model1.DataDictionary{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) DataDictionaryByUnionPk(ctx context.Context, dictionaryID string) (*model1.DataDictionary, error) {
	var rs model1.DataDictionary
	var m = &model1.DataDictionary{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := dictionaryID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", dictionaryID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
					_ = cacheAspect.SetNotExistUnionPkQueryCache(ctx, cacheKey, model1.DataDictionary{})
				}
				return &rs, nil
			}
			return nil, err
