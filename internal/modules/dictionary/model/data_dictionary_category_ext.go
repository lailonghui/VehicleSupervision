...
// 新建主键dataloader
func (t *DataDictionaryCategoryPkLoader) NewLoader(ctx context.Context) *DataDictionaryCategoryPkLoader {
	return &DataDictionaryCategoryPkLoader{
		wait:     2 * time.Millisecond,
		maxBatch: 100,
		fetch: func(keys []string) ([]*DataDictionaryCategory, []error) {
			var rs []*DataDictionaryCategory = make([]*DataDictionaryCategory, len(keys))
			...
			if middle.IsEnableGqlCache(ctx) {
				...
				for i := range keys {
					...
					go func(i int) {
						...
						// 如果启用缓存，则从缓存中查询数据
						var entity DataDictionaryCategory
						cacheKey := keys[i]
						if cacheErr == nil {
...
// 新建联合主键dataloader
func (t *DataDictionaryCategoryUnionPkLoader) NewLoader(ctx context.Context) *DataDictionaryCategoryUnionPkLoader {
