...
// 新建主键dataloader
func (t *LimitSpeedPlanDetailPkLoader) NewLoader(ctx context.Context) *LimitSpeedPlanDetailPkLoader {
	return &LimitSpeedPlanDetailPkLoader{
		wait:     2 * time.Millisecond,
		maxBatch: 100,
		fetch: func(keys []string) ([]*LimitSpeedPlanDetail, []error) {
			var rs []*LimitSpeedPlanDetail = make([]*LimitSpeedPlanDetail, len(keys))
			...
			if middle.IsEnableGqlCache(ctx) {
				...
				for i := range keys {
					...
					go func(i int) {
						...
						// 如果启用缓存，则从缓存中查询数据
						var entity LimitSpeedPlanDetail
						cacheKey := keys[i]
						if cacheErr == nil {
...
// 新建联合主键dataloader
func (t *LimitSpeedPlanDetailUnionPkLoader) NewLoader(ctx context.Context) *LimitSpeedPlanDetailUnionPkLoader {
