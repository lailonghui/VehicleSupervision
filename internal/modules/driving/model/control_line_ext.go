...
// 新建主键dataloader
func (t *ControlLinePkLoader) NewLoader(ctx context.Context) *ControlLinePkLoader {
	return &ControlLinePkLoader{
		wait:     2 * time.Millisecond,
		maxBatch: 100,
		fetch: func(keys []string) ([]*ControlLine, []error) {
			var rs []*ControlLine = make([]*ControlLine, len(keys))
			var m ControlLine
			wg := sync.WaitGroup{}
...
// 新建联合主键dataloader
func (t *ControlLineUnionPkLoader) NewLoader(ctx context.Context) *ControlLineUnionPkLoader {
