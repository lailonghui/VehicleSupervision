...
// 新建主键dataloader
func (t *EcdLinePkLoader) NewLoader(ctx context.Context) *EcdLinePkLoader {
	return &EcdLinePkLoader{
		wait:     2 * time.Millisecond,
		maxBatch: 100,
		fetch: func(keys []string) ([]*EcdLine, []error) {
			var rs []*EcdLine = make([]*EcdLine, len(keys))
			var m EcdLine
			wg := sync.WaitGroup{}
...
// 新建联合主键dataloader
func (t *EcdLineUnionPkLoader) NewLoader(ctx context.Context) *EcdLineUnionPkLoader {
