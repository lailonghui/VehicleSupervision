...
// 新建主键dataloader
func (t *VehicleNightTravelRecordPkLoader) NewLoader(ctx context.Context) *VehicleNightTravelRecordPkLoader {
	return &VehicleNightTravelRecordPkLoader{
		wait:     2 * time.Millisecond,
		maxBatch: 100,
		fetch: func(keys []string) ([]*VehicleNightTravelRecord, []error) {
			var rs []*VehicleNightTravelRecord = make([]*VehicleNightTravelRecord, len(keys))
			...
			if middle.IsEnableGqlCache(ctx) {
				...
				for i := range keys {
					...
					go func(i int) {
						...
						// 如果启用缓存，则从缓存中查询数据
						var entity VehicleNightTravelRecord
						cacheKey := keys[i]
						if cacheErr == nil {
...
// 新建联合主键dataloader
func (t *VehicleNightTravelRecordUnionPkLoader) NewLoader(ctx context.Context) *VehicleNightTravelRecordUnionPkLoader {
