...
func (r *mutationResolver) DeleteControlLineTimeByUnionPk(ctx context.Context, unionId string) (*model1.ControlLineTime, error) {
	preloads := util.GetPreloads(ctx)
	var rs model1.ControlLineTime
	tx := db.DB.Model(&model1.ControlLineTime{})
	if len(preloads) > 0 {
...
func (r *mutationResolver) UpdateControlLineTimeByUnionPk(ctx context.Context, inc *model.ControlLineTimeIncInput, set *model.ControlLineTimeSetInput, unionId string) (*model1.ControlLineTime, error) {
	var rs model1.ControlLineTime
...
func (r *queryResolver) ControlLineTimeByUnionPk(ctx context.Context, unionId string) (*model1.ControlLineTime, error) {
	var rs model1.ControlLineTime
	tx := db.DB.Model(&model1.ControlLineTime{}).Select(util.GetTopPreloads(ctx)).Where(rs.UnionPrimaryColumnName()+" = ?", unionId).First(&rs)
