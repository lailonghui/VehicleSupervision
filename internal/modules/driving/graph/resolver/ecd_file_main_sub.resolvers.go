...
func (r *mutationResolver) DeleteEcdFileMainSubByUnionPk(ctx context.Context, fileMainSubID string) (*model1.EcdFileMainSub, error) {
	var rs model1.EcdFileMainSub
	m := &model1.EcdFileMainSub{}
	tx := db.DB.Model(&model1.EcdFileMainSub{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", fileMainSubID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertEcdFileMainSubOne(ctx context.Context, objects model.EcdFileMainSubInsertInput) (*model1.EcdFileMainSub, error) {
...
func (r *mutationResolver) UpdateEcdFileMainSubByUnionPk(ctx context.Context, inc *model.EcdFileMainSubIncInput, set *model.EcdFileMainSubSetInput, fileMainSubID string) (*model1.EcdFileMainSub, error) {
	var rs model1.EcdFileMainSub
	var m = &model1.EcdFileMainSub{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", fileMainSubID)
	qt := util.NewQueryTranslator(tx, &model1.EcdFileMainSub{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) EcdFileMainSubByUnionPk(ctx context.Context, fileMainSubID string) (*model1.EcdFileMainSub, error) {
	var rs model1.EcdFileMainSub
	var m = &model1.EcdFileMainSub{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := fileMainSubID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", fileMainSubID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
					_ = cacheAspect.SetNotExistUnionPkQueryCache(ctx, cacheKey, model1.EcdFileMainSub{})
				}
				return &rs, nil
			}
			return nil, err
