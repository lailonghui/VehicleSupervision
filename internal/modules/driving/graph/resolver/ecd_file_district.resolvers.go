...
func (r *mutationResolver) DeleteEcdFileDistrictByUnionPk(ctx context.Context, unionId string) (*model1.EcdFileDistrict, error) {
	preloads := util.GetPreloads(ctx)
	var rs model1.EcdFileDistrict
	tx := db.DB.Model(&model1.EcdFileDistrict{})
	if len(preloads) > 0 {
...
func (r *mutationResolver) UpdateEcdFileDistrictByUnionPk(ctx context.Context, inc *model.EcdFileDistrictIncInput, set *model.EcdFileDistrictSetInput, unionId string) (*model1.EcdFileDistrict, error) {
	var rs model1.EcdFileDistrict
...
func (r *queryResolver) EcdFileDistrictByUnionPk(ctx context.Context, unionId string) (*model1.EcdFileDistrict, error) {
	var rs model1.EcdFileDistrict
	tx := db.DB.Model(&model1.EcdFileDistrict{}).Select(util.GetTopPreloads(ctx)).Where(rs.UnionPrimaryColumnName()+" = ?", unionId).First(&rs)
