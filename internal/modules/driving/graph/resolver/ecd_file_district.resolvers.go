...
func (r *mutationResolver) DeleteEcdFileDistrictByUnionPk(ctx context.Context, fileDistrictID string) (*model1.EcdFileDistrict, error) {
	var rs model1.EcdFileDistrict
	m := &model1.EcdFileDistrict{}
	tx := db.DB.Model(&model1.EcdFileDistrict{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", fileDistrictID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertEcdFileDistrictOne(ctx context.Context, objects model.EcdFileDistrictInsertInput) (*model1.EcdFileDistrict, error) {
...
func (r *mutationResolver) UpdateEcdFileDistrictByUnionPk(ctx context.Context, inc *model.EcdFileDistrictIncInput, set *model.EcdFileDistrictSetInput, fileDistrictID string) (*model1.EcdFileDistrict, error) {
	var rs model1.EcdFileDistrict
	var m = &model1.EcdFileDistrict{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", fileDistrictID)
	qt := util.NewQueryTranslator(tx, &model1.EcdFileDistrict{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) EcdFileDistrictByUnionPk(ctx context.Context, fileDistrictID string) (*model1.EcdFileDistrict, error) {
	var rs model1.EcdFileDistrict
	var m = &model1.EcdFileDistrict{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := fileDistrictID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", fileDistrictID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
					_ = cacheAspect.SetNotExistUnionPkQueryCache(ctx, cacheKey, model1.EcdFileDistrict{})
				}
				return &rs, nil
			}
			return nil, err
