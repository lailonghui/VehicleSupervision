...
func (r *mutationResolver) DeleteEcdFileLinePointByUnionPk(ctx context.Context, linePointID string) (*model1.EcdFileLinePoint, error) {
	var rs model1.EcdFileLinePoint
	m := &model1.EcdFileLinePoint{}
	tx := db.DB.Model(&model1.EcdFileLinePoint{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", linePointID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertEcdFileLinePointOne(ctx context.Context, objects model.EcdFileLinePointInsertInput) (*model1.EcdFileLinePoint, error) {
...
func (r *mutationResolver) UpdateEcdFileLinePointByUnionPk(ctx context.Context, inc *model.EcdFileLinePointIncInput, set *model.EcdFileLinePointSetInput, linePointID string) (*model1.EcdFileLinePoint, error) {
	var rs model1.EcdFileLinePoint
	var m = &model1.EcdFileLinePoint{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", linePointID)
	qt := util.NewQueryTranslator(tx, &model1.EcdFileLinePoint{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) EcdFileLinePointByUnionPk(ctx context.Context, linePointID string) (*model1.EcdFileLinePoint, error) {
	var rs model1.EcdFileLinePoint
	var m = &model1.EcdFileLinePoint{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := linePointID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", linePointID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
					_ = cacheAspect.SetNotExistUnionPkQueryCache(ctx, cacheKey, model1.EcdFileLinePoint{})
				}
				return &rs, nil
			}
			return nil, err
