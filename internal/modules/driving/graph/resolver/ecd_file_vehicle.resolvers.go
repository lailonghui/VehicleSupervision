...
func (r *mutationResolver) DeleteEcdFileVehicleByUnionPk(ctx context.Context, unionId string) (*model1.EcdFileVehicle, error) {
	preloads := util.GetPreloads(ctx)
	var rs model1.EcdFileVehicle
	tx := db.DB.Model(&model1.EcdFileVehicle{})
	if len(preloads) > 0 {
...
func (r *mutationResolver) UpdateEcdFileVehicleByUnionPk(ctx context.Context, inc *model.EcdFileVehicleIncInput, set *model.EcdFileVehicleSetInput, unionId string) (*model1.EcdFileVehicle, error) {
	var rs model1.EcdFileVehicle
...
func (r *queryResolver) EcdFileVehicleByUnionPk(ctx context.Context, unionId string) (*model1.EcdFileVehicle, error) {
	var rs model1.EcdFileVehicle
	tx := db.DB.Model(&model1.EcdFileVehicle{}).Select(util.GetTopPreloads(ctx)).Where(rs.UnionPrimaryColumnName()+" = ?", unionId).First(&rs)
