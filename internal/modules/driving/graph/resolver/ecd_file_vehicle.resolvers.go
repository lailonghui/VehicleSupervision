...
func (r *mutationResolver) DeleteEcdFileVehicleByUnionPk(ctx context.Context, fileVehicleID string) (*model1.EcdFileVehicle, error) {
	var rs model1.EcdFileVehicle
	m := &model1.EcdFileVehicle{}
	tx := db.DB.Model(&model1.EcdFileVehicle{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", fileVehicleID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertEcdFileVehicleOne(ctx context.Context, objects model.EcdFileVehicleInsertInput) (*model1.EcdFileVehicle, error) {
...
func (r *mutationResolver) UpdateEcdFileVehicleByUnionPk(ctx context.Context, inc *model.EcdFileVehicleIncInput, set *model.EcdFileVehicleSetInput, fileVehicleID string) (*model1.EcdFileVehicle, error) {
	var rs model1.EcdFileVehicle
	var m = &model1.EcdFileVehicle{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", fileVehicleID)
	qt := util.NewQueryTranslator(tx, &model1.EcdFileVehicle{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) EcdFileVehicleByUnionPk(ctx context.Context, fileVehicleID string) (*model1.EcdFileVehicle, error) {
	var rs model1.EcdFileVehicle
	var m = &model1.EcdFileVehicle{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := fileVehicleID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", fileVehicleID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
					_ = cacheAspect.SetNotExistUnionPkQueryCache(ctx, cacheKey, model1.EcdFileVehicle{})
				}
				return &rs, nil
			}
			return nil, err
