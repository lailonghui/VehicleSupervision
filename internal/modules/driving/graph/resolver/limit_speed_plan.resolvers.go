...
func (r *mutationResolver) DeleteLimitSpeedPlanByUnionPk(ctx context.Context, limitSpeedPlanID string) (*model1.LimitSpeedPlan, error) {
	var rs model1.LimitSpeedPlan
	m := &model1.LimitSpeedPlan{}
	tx := db.DB.Model(&model1.LimitSpeedPlan{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", limitSpeedPlanID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertLimitSpeedPlanOne(ctx context.Context, objects model.LimitSpeedPlanInsertInput) (*model1.LimitSpeedPlan, error) {
...
func (r *mutationResolver) UpdateLimitSpeedPlanByUnionPk(ctx context.Context, inc *model.LimitSpeedPlanIncInput, set *model.LimitSpeedPlanSetInput, limitSpeedPlanID string) (*model1.LimitSpeedPlan, error) {
	var rs model1.LimitSpeedPlan
	var m = &model1.LimitSpeedPlan{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", limitSpeedPlanID)
	qt := util.NewQueryTranslator(tx, &model1.LimitSpeedPlan{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) LimitSpeedPlanByUnionPk(ctx context.Context, limitSpeedPlanID string) (*model1.LimitSpeedPlan, error) {
	var rs model1.LimitSpeedPlan
	var m = &model1.LimitSpeedPlan{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := limitSpeedPlanID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", limitSpeedPlanID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
					_ = cacheAspect.SetNotExistUnionPkQueryCache(ctx, cacheKey, model1.LimitSpeedPlan{})
				}
				return &rs, nil
			}
			return nil, err
