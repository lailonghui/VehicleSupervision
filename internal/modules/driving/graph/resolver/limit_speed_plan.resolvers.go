...
func (r *mutationResolver) DeleteLimitSpeedPlanByUnionPk(ctx context.Context, unionId string) (*model1.LimitSpeedPlan, error) {
	preloads := util.GetPreloads(ctx)
	var rs model1.LimitSpeedPlan
	tx := db.DB.Model(&model1.LimitSpeedPlan{})
	if len(preloads) > 0 {
...
func (r *mutationResolver) UpdateLimitSpeedPlanByUnionPk(ctx context.Context, inc *model.LimitSpeedPlanIncInput, set *model.LimitSpeedPlanSetInput, unionId string) (*model1.LimitSpeedPlan, error) {
	var rs model1.LimitSpeedPlan
...
func (r *queryResolver) LimitSpeedPlanByUnionPk(ctx context.Context, unionId string) (*model1.LimitSpeedPlan, error) {
	var rs model1.LimitSpeedPlan
	tx := db.DB.Model(&model1.LimitSpeedPlan{}).Select(util.GetTopPreloads(ctx)).Where(rs.UnionPrimaryColumnName()+" = ?", unionId).First(&rs)
