...
func (r *mutationResolver) DeleteEcdFileCheckHisByUnionPk(ctx context.Context, unionId string) (*model1.EcdFileCheckHis, error) {
	preloads := util.GetPreloads(ctx)
	var rs model1.EcdFileCheckHis
	tx := db.DB.Model(&model1.EcdFileCheckHis{})
	if len(preloads) > 0 {
...
func (r *mutationResolver) UpdateEcdFileCheckHisByUnionPk(ctx context.Context, inc *model.EcdFileCheckHisIncInput, set *model.EcdFileCheckHisSetInput, unionId string) (*model1.EcdFileCheckHis, error) {
	var rs model1.EcdFileCheckHis
...
func (r *queryResolver) EcdFileCheckHisByUnionPk(ctx context.Context, unionId string) (*model1.EcdFileCheckHis, error) {
	var rs model1.EcdFileCheckHis
	tx := db.DB.Model(&model1.EcdFileCheckHis{}).Select(util.GetTopPreloads(ctx)).Where(rs.UnionPrimaryColumnName()+" = ?", unionId).First(&rs)
