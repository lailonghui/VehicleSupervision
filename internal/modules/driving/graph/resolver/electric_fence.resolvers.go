...
func (r *mutationResolver) DeleteElectricFenceByUnionPk(ctx context.Context, electricFenceID string) (*model1.ElectricFence, error) {
	var rs model1.ElectricFence
	m := &model1.ElectricFence{}
	tx := db.DB.Model(&model1.ElectricFence{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", electricFenceID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertElectricFenceOne(ctx context.Context, objects model.ElectricFenceInsertInput) (*model1.ElectricFence, error) {
...
func (r *mutationResolver) UpdateElectricFenceByUnionPk(ctx context.Context, inc *model.ElectricFenceIncInput, set *model.ElectricFenceSetInput, electricFenceID string) (*model1.ElectricFence, error) {
	var rs model1.ElectricFence
	var m = &model1.ElectricFence{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", electricFenceID)
	qt := util.NewQueryTranslator(tx, &model1.ElectricFence{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) ElectricFenceByUnionPk(ctx context.Context, electricFenceID string) (*model1.ElectricFence, error) {
	var rs model1.ElectricFence
	var m = &model1.ElectricFence{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := electricFenceID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", electricFenceID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
					_ = cacheAspect.SetNotExistUnionPkQueryCache(ctx, cacheKey, model1.ElectricFence{})
				}
				return &rs, nil
			}
			return nil, err
