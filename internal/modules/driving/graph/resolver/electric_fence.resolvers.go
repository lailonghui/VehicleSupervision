...
func (r *mutationResolver) DeleteElectricFenceByUnionPk(ctx context.Context, unionId string) (*model1.ElectricFence, error) {
	preloads := util.GetPreloads(ctx)
	var rs model1.ElectricFence
	tx := db.DB.Model(&model1.ElectricFence{})
	if len(preloads) > 0 {
...
func (r *mutationResolver) UpdateElectricFenceByUnionPk(ctx context.Context, inc *model.ElectricFenceIncInput, set *model.ElectricFenceSetInput, unionId string) (*model1.ElectricFence, error) {
	var rs model1.ElectricFence
...
func (r *queryResolver) ElectricFenceByUnionPk(ctx context.Context, unionId string) (*model1.ElectricFence, error) {
	var rs model1.ElectricFence
	tx := db.DB.Model(&model1.ElectricFence{}).Select(util.GetTopPreloads(ctx)).Where(rs.UnionPrimaryColumnName()+" = ?", unionId).First(&rs)
