...
func (r *mutationResolver) DeleteLimitSpeedPlanDetail(ctx context.Context, where model.LimitSpeedPlanDetailBoolExp) (*model.LimitSpeedPlanDetailMutationResponse, error) {
	...
	var m = &model1.LimitSpeedPlanDetail{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteLimitSpeedPlanDetailByUnionPk(ctx context.Context, detailID string) (*model1.LimitSpeedPlanDetail, error) {
	var rs model1.LimitSpeedPlanDetail
	m := &model1.LimitSpeedPlanDetail{}
	tx := db.DB.Model(&model1.LimitSpeedPlanDetail{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", detailID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertLimitSpeedPlanDetailOne(ctx context.Context, objects model.LimitSpeedPlanDetailInsertInput) (*model1.LimitSpeedPlanDetail, error) {
...
func (r *mutationResolver) UpdateLimitSpeedPlanDetailByUnionPk(ctx context.Context, inc *model.LimitSpeedPlanDetailIncInput, set *model.LimitSpeedPlanDetailSetInput, detailID string) (*model1.LimitSpeedPlanDetail, error) {
	var rs model1.LimitSpeedPlanDetail
	var m = &model1.LimitSpeedPlanDetail{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", detailID)
	qt := util.NewQueryTranslator(tx, &model1.LimitSpeedPlanDetail{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) LimitSpeedPlanDetailByUnionPk(ctx context.Context, detailID string) (*model1.LimitSpeedPlanDetail, error) {
	var rs model1.LimitSpeedPlanDetail
	var m = &model1.LimitSpeedPlanDetail{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", detailID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
					_ = cacheAspect.SetNotExistUnionPkQueryCache(ctx, cacheKey, model1.LimitSpeedPlanDetail{})
				}
				return &rs, nil
			}
			return nil, err
