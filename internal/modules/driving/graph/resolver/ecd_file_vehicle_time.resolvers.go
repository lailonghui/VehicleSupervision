...
func (r *mutationResolver) DeleteEcdFileVehicleTime(ctx context.Context, where model.EcdFileVehicleTimeBoolExp) (*model.EcdFileVehicleTimeMutationResponse, error) {
	...
	var m = &model1.EcdFileVehicleTime{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteEcdFileVehicleTimeByUnionPk(ctx context.Context, vehicleTimeID string) (*model1.EcdFileVehicleTime, error) {
	var rs model1.EcdFileVehicleTime
	m := &model1.EcdFileVehicleTime{}
	tx := db.DB.Model(&model1.EcdFileVehicleTime{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", vehicleTimeID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertEcdFileVehicleTimeOne(ctx context.Context, objects model.EcdFileVehicleTimeInsertInput) (*model1.EcdFileVehicleTime, error) {
...
func (r *mutationResolver) UpdateEcdFileVehicleTimeByUnionPk(ctx context.Context, inc *model.EcdFileVehicleTimeIncInput, set *model.EcdFileVehicleTimeSetInput, vehicleTimeID string) (*model1.EcdFileVehicleTime, error) {
	var rs model1.EcdFileVehicleTime
	var m = &model1.EcdFileVehicleTime{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", vehicleTimeID)
	qt := util.NewQueryTranslator(tx, &model1.EcdFileVehicleTime{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) EcdFileVehicleTimeByUnionPk(ctx context.Context, vehicleTimeID string) (*model1.EcdFileVehicleTime, error) {
	var rs model1.EcdFileVehicleTime
	var m = &model1.EcdFileVehicleTime{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := vehicleTimeID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", vehicleTimeID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
					_ = cacheAspect.SetNotExistUnionPkQueryCache(ctx, cacheKey, model1.EcdFileVehicleTime{})
				}
				return &rs, nil
			}
			return nil, err
