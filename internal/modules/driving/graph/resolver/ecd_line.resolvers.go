...
func (r *mutationResolver) DeleteEcdLineByUnionPk(ctx context.Context, lineID string) (*model1.EcdLine, error) {
	var rs model1.EcdLine
	m := &model1.EcdLine{}
	tx := db.DB.Model(&model1.EcdLine{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", lineID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertEcdLineOne(ctx context.Context, objects model.EcdLineInsertInput) (*model1.EcdLine, error) {
	rs := &model1.EcdLine{}
	m := &model1.EcdLine{}
	// 插入记录
...
func (r *mutationResolver) UpdateEcdLineByUnionPk(ctx context.Context, inc *model.EcdLineIncInput, set *model.EcdLineSetInput, lineID string) (*model1.EcdLine, error) {
	var rs model1.EcdLine
	var m = &model1.EcdLine{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", lineID)
	qt := util.NewQueryTranslator(tx, &model1.EcdLine{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) EcdLineByUnionPk(ctx context.Context, lineID string) (*model1.EcdLine, error) {
	var rs model1.EcdLine
	var m = &model1.EcdLine{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", lineID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
					_ = cacheAspect.SetNotExistUnionPkQueryCache(ctx, cacheKey, model1.EcdLine{})
				}
				return &rs, nil
			}
			return nil, err
