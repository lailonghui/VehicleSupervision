...
func (r *mutationResolver) DeleteLimitSpeedLineTime(ctx context.Context, where model.LimitSpeedLineTimeBoolExp) (*model.LimitSpeedLineTimeMutationResponse, error) {
	...
	var m = &model1.LimitSpeedLineTime{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteLimitSpeedLineTimeByUnionPk(ctx context.Context, limitSpeedLineTimeID string) (*model1.LimitSpeedLineTime, error) {
	var rs model1.LimitSpeedLineTime
	m := &model1.LimitSpeedLineTime{}
	tx := db.DB.Model(&model1.LimitSpeedLineTime{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", limitSpeedLineTimeID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertLimitSpeedLineTimeOne(ctx context.Context, objects model.LimitSpeedLineTimeInsertInput) (*model1.LimitSpeedLineTime, error) {
...
func (r *mutationResolver) UpdateLimitSpeedLineTimeByUnionPk(ctx context.Context, inc *model.LimitSpeedLineTimeIncInput, set *model.LimitSpeedLineTimeSetInput, limitSpeedLineTimeID string) (*model1.LimitSpeedLineTime, error) {
	var rs model1.LimitSpeedLineTime
	var m = &model1.LimitSpeedLineTime{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", limitSpeedLineTimeID)
	qt := util.NewQueryTranslator(tx, &model1.LimitSpeedLineTime{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) LimitSpeedLineTimeByUnionPk(ctx context.Context, limitSpeedLineTimeID string) (*model1.LimitSpeedLineTime, error) {
	var rs model1.LimitSpeedLineTime
	var m = &model1.LimitSpeedLineTime{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := limitSpeedLineTimeID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", limitSpeedLineTimeID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
					_ = cacheAspect.SetNotExistUnionPkQueryCache(ctx, cacheKey, model1.LimitSpeedLineTime{})
				}
				return &rs, nil
			}
			return nil, err
