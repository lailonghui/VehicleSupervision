...
func (r *mutationResolver) DeleteEcdFileMainByUnionPk(ctx context.Context, fileMainID string) (*model1.EcdFileMain, error) {
	var rs model1.EcdFileMain
	m := &model1.EcdFileMain{}
	tx := db.DB.Model(&model1.EcdFileMain{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", fileMainID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertEcdFileMainOne(ctx context.Context, objects model.EcdFileMainInsertInput) (*model1.EcdFileMain, error) {
...
func (r *mutationResolver) UpdateEcdFileMainByUnionPk(ctx context.Context, inc *model.EcdFileMainIncInput, set *model.EcdFileMainSetInput, fileMainID string) (*model1.EcdFileMain, error) {
	var rs model1.EcdFileMain
	var m = &model1.EcdFileMain{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", fileMainID)
	qt := util.NewQueryTranslator(tx, &model1.EcdFileMain{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) EcdFileMainByUnionPk(ctx context.Context, fileMainID string) (*model1.EcdFileMain, error) {
	var rs model1.EcdFileMain
	var m = &model1.EcdFileMain{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", fileMainID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
					_ = cacheAspect.SetNotExistUnionPkQueryCache(ctx, cacheKey, model1.EcdFileMain{})
				}
				return &rs, nil
			}
			return nil, err
