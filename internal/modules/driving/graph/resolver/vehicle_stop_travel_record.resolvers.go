...
func (r *mutationResolver) DeleteVehicleStopTravelRecordByUnionPk(ctx context.Context, unionId string) (*model1.VehicleStopTravelRecord, error) {
	preloads := util.GetPreloads(ctx)
	var rs model1.VehicleStopTravelRecord
	tx := db.DB.Model(&model1.VehicleStopTravelRecord{})
	if len(preloads) > 0 {
...
func (r *mutationResolver) UpdateVehicleStopTravelRecordByUnionPk(ctx context.Context, inc *model.VehicleStopTravelRecordIncInput, set *model.VehicleStopTravelRecordSetInput, unionId string) (*model1.VehicleStopTravelRecord, error) {
	var rs model1.VehicleStopTravelRecord
...
func (r *queryResolver) VehicleStopTravelRecordByUnionPk(ctx context.Context, unionId string) (*model1.VehicleStopTravelRecord, error) {
	var rs model1.VehicleStopTravelRecord
	tx := db.DB.Model(&model1.VehicleStopTravelRecord{}).Select(util.GetTopPreloads(ctx)).Where(rs.UnionPrimaryColumnName()+" = ?", unionId).First(&rs)
