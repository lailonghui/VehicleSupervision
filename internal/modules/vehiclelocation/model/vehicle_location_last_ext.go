...
// 新建主键dataloader
func (t *VehicleLocationLastPkLoader) NewLoader(ctx context.Context) *VehicleLocationLastPkLoader {
	return &VehicleLocationLastPkLoader{
		wait:     2 * time.Millisecond,
		maxBatch: 100,
		fetch: func(keys []string) ([]*VehicleLocationLast, []error) {
			var rs []*VehicleLocationLast = make([]*VehicleLocationLast, len(keys))
			...
			if middle.IsEnableGqlCache(ctx) {
				...
				for i := range keys {
					...
					go func(i int) {
						...
						// 如果启用缓存，则从缓存中查询数据
						var entity VehicleLocationLast
						cacheKey := keys[i]
						if cacheErr == nil {
