...
func (r *mutationResolver) DeleteRideHailingDriverVerify(ctx context.Context, where model.RideHailingDriverVerifyBoolExp) (*model.RideHailingDriverVerifyMutationResponse, error) {
	...
	var m = &model1.RideHailingDriverVerify{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteRideHailingDriverVerifyByPk(ctx context.Context, id int64) (*model1.RideHailingDriverVerify, error) {
	var rs model1.RideHailingDriverVerify
	m := &model1.RideHailingDriverVerify{}
	tx := db.DB.Model(m)
	// 查询记录
...
func (r *mutationResolver) DeleteRideHailingDriverVerifyByUnionPk(ctx context.Context, rideHailingDriverVerifyID string) (*model1.RideHailingDriverVerify, error) {
	var rs model1.RideHailingDriverVerify
	m := &model1.RideHailingDriverVerify{}
	tx := db.DB.Model(&model1.RideHailingDriverVerify{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", rideHailingDriverVerifyID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertRideHailingDriverVerifyOne(ctx context.Context, objects model.RideHailingDriverVerifyInsertInput) (*model1.RideHailingDriverVerify, error) {
...
func (r *mutationResolver) UpdateRideHailingDriverVerifyByUnionPk(ctx context.Context, inc *model.RideHailingDriverVerifyIncInput, set *model.RideHailingDriverVerifySetInput, rideHailingDriverVerifyID string) (*model1.RideHailingDriverVerify, error) {
	var rs model1.RideHailingDriverVerify
	var m = &model1.RideHailingDriverVerify{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", rideHailingDriverVerifyID)
	qt := util.NewQueryTranslator(tx, &model1.RideHailingDriverVerify{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) RideHailingDriverVerifyByUnionPk(ctx context.Context, rideHailingDriverVerifyID string) (*model1.RideHailingDriverVerify, error) {
	var rs model1.RideHailingDriverVerify
	var m = &model1.RideHailingDriverVerify{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := rideHailingDriverVerifyID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", rideHailingDriverVerifyID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
					_ = cacheAspect.SetNotExistUnionPkQueryCache(ctx, cacheKey, model1.RideHailingDriverVerify{})
				}
				return &rs, nil
			}
			return nil, err
