...
func (r *mutationResolver) DeleteRideHailingDriver(ctx context.Context, where model.RideHailingDriverBoolExp) (*model.RideHailingDriverMutationResponse, error) {
	...
	var m = &model1.RideHailingDriver{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteRideHailingDriverByUnionPk(ctx context.Context, rideHailingDriverID string) (*model1.RideHailingDriver, error) {
	var rs model1.RideHailingDriver
	m := &model1.RideHailingDriver{}
	tx := db.DB.Model(&model1.RideHailingDriver{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", rideHailingDriverID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertRideHailingDriverOne(ctx context.Context, objects model.RideHailingDriverInsertInput) (*model1.RideHailingDriver, error) {
...
func (r *mutationResolver) UpdateRideHailingDriverByUnionPk(ctx context.Context, inc *model.RideHailingDriverIncInput, set *model.RideHailingDriverSetInput, rideHailingDriverID string) (*model1.RideHailingDriver, error) {
	var rs model1.RideHailingDriver
	var m = &model1.RideHailingDriver{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", rideHailingDriverID)
	qt := util.NewQueryTranslator(tx, &model1.RideHailingDriver{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) RideHailingDriverByUnionPk(ctx context.Context, rideHailingDriverID string) (*model1.RideHailingDriver, error) {
	var rs model1.RideHailingDriver
	var m = &model1.RideHailingDriver{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := rideHailingDriverID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", rideHailingDriverID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
					_ = cacheAspect.SetNotExistUnionPkQueryCache(ctx, cacheKey, model1.RideHailingDriver{})
				}
				return &rs, nil
			}
			return nil, err
