...
func (r *mutationResolver) DeleteNewMuckTruckPhoto(ctx context.Context, where model.NewMuckTruckPhotoBoolExp) (*model.NewMuckTruckPhotoMutationResponse, error) {
	...
	var m = &model1.NewMuckTruckPhoto{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteNewMuckTruckPhotoByUnionPk(ctx context.Context, newMuckTruckPhotoID string) (*model1.NewMuckTruckPhoto, error) {
	var rs model1.NewMuckTruckPhoto
	m := &model1.NewMuckTruckPhoto{}
	tx := db.DB.Model(&model1.NewMuckTruckPhoto{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", newMuckTruckPhotoID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertNewMuckTruckPhotoOne(ctx context.Context, objects model.NewMuckTruckPhotoInsertInput) (*model1.NewMuckTruckPhoto, error) {
...
func (r *mutationResolver) UpdateNewMuckTruckPhotoByUnionPk(ctx context.Context, inc *model.NewMuckTruckPhotoIncInput, set *model.NewMuckTruckPhotoSetInput, newMuckTruckPhotoID string) (*model1.NewMuckTruckPhoto, error) {
	var rs model1.NewMuckTruckPhoto
	var m = &model1.NewMuckTruckPhoto{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", newMuckTruckPhotoID)
	qt := util.NewQueryTranslator(tx, &model1.NewMuckTruckPhoto{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) NewMuckTruckPhotoByUnionPk(ctx context.Context, newMuckTruckPhotoID string) (*model1.NewMuckTruckPhoto, error) {
	var rs model1.NewMuckTruckPhoto
	var m = &model1.NewMuckTruckPhoto{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := newMuckTruckPhotoID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", newMuckTruckPhotoID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
