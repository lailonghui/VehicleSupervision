...
func (r *mutationResolver) DeleteNewMuckTruckInfoByUnionPk(ctx context.Context, newMuckTruckInfoID string) (*model1.NewMuckTruckInfo, error) {
	var rs model1.NewMuckTruckInfo
	m := &model1.NewMuckTruckInfo{}
	tx := db.DB.Model(&model1.NewMuckTruckInfo{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", newMuckTruckInfoID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertNewMuckTruckInfoOne(ctx context.Context, objects model.NewMuckTruckInfoInsertInput) (*model1.NewMuckTruckInfo, error) {
...
func (r *mutationResolver) UpdateNewMuckTruckInfoByUnionPk(ctx context.Context, inc *model.NewMuckTruckInfoIncInput, set *model.NewMuckTruckInfoSetInput, newMuckTruckInfoID string) (*model1.NewMuckTruckInfo, error) {
	var rs model1.NewMuckTruckInfo
	var m = &model1.NewMuckTruckInfo{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", newMuckTruckInfoID)
	qt := util.NewQueryTranslator(tx, &model1.NewMuckTruckInfo{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) NewMuckTruckInfoByUnionPk(ctx context.Context, newMuckTruckInfoID string) (*model1.NewMuckTruckInfo, error) {
	var rs model1.NewMuckTruckInfo
	var m = &model1.NewMuckTruckInfo{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := newMuckTruckInfoID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", newMuckTruckInfoID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
