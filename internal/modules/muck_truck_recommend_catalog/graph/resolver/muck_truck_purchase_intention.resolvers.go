...
func (r *mutationResolver) DeleteMuckTruckPurchaseIntention(ctx context.Context, where model.MuckTruckPurchaseIntentionBoolExp) (*model.MuckTruckPurchaseIntentionMutationResponse, error) {
	...
	var m = &model1.MuckTruckPurchaseIntention{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteMuckTruckPurchaseIntentionByPk(ctx context.Context, id int64) (*model1.MuckTruckPurchaseIntention, error) {
	var rs model1.MuckTruckPurchaseIntention
	m := &model1.MuckTruckPurchaseIntention{}
	tx := db.DB.Model(m)
	// 查询记录
...
func (r *mutationResolver) DeleteMuckTruckPurchaseIntentionByUnionPk(ctx context.Context, muckTruckPurchaseIntentionID string) (*model1.MuckTruckPurchaseIntention, error) {
	var rs model1.MuckTruckPurchaseIntention
	m := &model1.MuckTruckPurchaseIntention{}
	tx := db.DB.Model(&model1.MuckTruckPurchaseIntention{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", muckTruckPurchaseIntentionID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertMuckTruckPurchaseIntentionOne(ctx context.Context, objects model.MuckTruckPurchaseIntentionInsertInput) (*model1.MuckTruckPurchaseIntention, error) {
...
func (r *mutationResolver) UpdateMuckTruckPurchaseIntentionByUnionPk(ctx context.Context, inc *model.MuckTruckPurchaseIntentionIncInput, set *model.MuckTruckPurchaseIntentionSetInput, muckTruckPurchaseIntentionID string) (*model1.MuckTruckPurchaseIntention, error) {
	var rs model1.MuckTruckPurchaseIntention
	var m = &model1.MuckTruckPurchaseIntention{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", muckTruckPurchaseIntentionID)
	qt := util.NewQueryTranslator(tx, &model1.MuckTruckPurchaseIntention{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) MuckTruckPurchaseIntentionByUnionPk(ctx context.Context, muckTruckPurchaseIntentionID string) (*model1.MuckTruckPurchaseIntention, error) {
	var rs model1.MuckTruckPurchaseIntention
	var m = &model1.MuckTruckPurchaseIntention{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := muckTruckPurchaseIntentionID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", muckTruckPurchaseIntentionID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
