...
func (r *mutationResolver) DeleteVehicleSaleRecord(ctx context.Context, where model.VehicleSaleRecordBoolExp) (*model.VehicleSaleRecordMutationResponse, error) {
	...
	var m = &model1.VehicleSaleRecord{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteVehicleSaleRecordByUnionPk(ctx context.Context, vehicleSaleRecordID string) (*model1.VehicleSaleRecord, error) {
	var rs model1.VehicleSaleRecord
	m := &model1.VehicleSaleRecord{}
	tx := db.DB.Model(&model1.VehicleSaleRecord{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", vehicleSaleRecordID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertVehicleSaleRecord(ctx context.Context, objects []*model.VehicleSaleRecordInsertInput) (*model.VehicleSaleRecordMutationResponse, error) {
	rs := make([]*model1.VehicleSaleRecord, 0)
	m := &model1.VehicleSaleRecord{}
	// 结构转换
...
func (r *mutationResolver) InsertVehicleSaleRecordOne(ctx context.Context, objects model.VehicleSaleRecordInsertInput) (*model1.VehicleSaleRecord, error) {
...
func (r *mutationResolver) UpdateVehicleSaleRecordByUnionPk(ctx context.Context, inc *model.VehicleSaleRecordIncInput, set *model.VehicleSaleRecordSetInput, vehicleSaleRecordID string) (*model1.VehicleSaleRecord, error) {
	var rs model1.VehicleSaleRecord
	var m = &model1.VehicleSaleRecord{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", vehicleSaleRecordID)
	qt := util.NewQueryTranslator(tx, &model1.VehicleSaleRecord{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) VehicleSaleRecordByUnionPk(ctx context.Context, vehicleSaleRecordID string) (*model1.VehicleSaleRecord, error) {
	var rs model1.VehicleSaleRecord
	var m = &model1.VehicleSaleRecord{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := vehicleSaleRecordID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", vehicleSaleRecordID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
