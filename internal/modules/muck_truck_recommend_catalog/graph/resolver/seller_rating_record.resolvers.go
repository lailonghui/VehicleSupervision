...
func (r *mutationResolver) DeleteSellerRatingRecord(ctx context.Context, where model.SellerRatingRecordBoolExp) (*model.SellerRatingRecordMutationResponse, error) {
	...
	var m = &model1.SellerRatingRecord{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteSellerRatingRecordByUnionPk(ctx context.Context, sellerRatingRecordID string) (*model1.SellerRatingRecord, error) {
	var rs model1.SellerRatingRecord
	m := &model1.SellerRatingRecord{}
	tx := db.DB.Model(&model1.SellerRatingRecord{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", sellerRatingRecordID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertSellerRatingRecordOne(ctx context.Context, objects model.SellerRatingRecordInsertInput) (*model1.SellerRatingRecord, error) {
...
func (r *mutationResolver) UpdateSellerRatingRecordByUnionPk(ctx context.Context, inc *model.SellerRatingRecordIncInput, set *model.SellerRatingRecordSetInput, sellerRatingRecordID string) (*model1.SellerRatingRecord, error) {
	var rs model1.SellerRatingRecord
	var m = &model1.SellerRatingRecord{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", sellerRatingRecordID)
	qt := util.NewQueryTranslator(tx, &model1.SellerRatingRecord{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) SellerRatingRecordByUnionPk(ctx context.Context, sellerRatingRecordID string) (*model1.SellerRatingRecord, error) {
	var rs model1.SellerRatingRecord
	var m = &model1.SellerRatingRecord{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := sellerRatingRecordID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", sellerRatingRecordID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
