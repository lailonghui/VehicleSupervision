...
func (r *mutationResolver) DeleteNewMuckTruckRecommendCatalog(ctx context.Context, where model.NewMuckTruckRecommendCatalogBoolExp) (*model.NewMuckTruckRecommendCatalogMutationResponse, error) {
	...
	var m = &model1.NewMuckTruckRecommendCatalog{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteNewMuckTruckRecommendCatalogByPk(ctx context.Context, id int64) (*model1.NewMuckTruckRecommendCatalog, error) {
	var rs model1.NewMuckTruckRecommendCatalog
	m := &model1.NewMuckTruckRecommendCatalog{}
	tx := db.DB.Model(m)
	// 查询记录
...
func (r *mutationResolver) DeleteNewMuckTruckRecommendCatalogByUnionPk(ctx context.Context, newMuckTruckRecommendCatalogID string) (*model1.NewMuckTruckRecommendCatalog, error) {
	var rs model1.NewMuckTruckRecommendCatalog
	m := &model1.NewMuckTruckRecommendCatalog{}
	tx := db.DB.Model(&model1.NewMuckTruckRecommendCatalog{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", newMuckTruckRecommendCatalogID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertNewMuckTruckRecommendCatalogOne(ctx context.Context, objects model.NewMuckTruckRecommendCatalogInsertInput) (*model1.NewMuckTruckRecommendCatalog, error) {
...
func (r *mutationResolver) UpdateNewMuckTruckRecommendCatalogByUnionPk(ctx context.Context, inc *model.NewMuckTruckRecommendCatalogIncInput, set *model.NewMuckTruckRecommendCatalogSetInput, newMuckTruckRecommendCatalogID string) (*model1.NewMuckTruckRecommendCatalog, error) {
	var rs model1.NewMuckTruckRecommendCatalog
	var m = &model1.NewMuckTruckRecommendCatalog{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", newMuckTruckRecommendCatalogID)
	qt := util.NewQueryTranslator(tx, &model1.NewMuckTruckRecommendCatalog{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) NewMuckTruckRecommendCatalogByUnionPk(ctx context.Context, newMuckTruckRecommendCatalogID string) (*model1.NewMuckTruckRecommendCatalog, error) {
	var rs model1.NewMuckTruckRecommendCatalog
	var m = &model1.NewMuckTruckRecommendCatalog{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := newMuckTruckRecommendCatalogID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", newMuckTruckRecommendCatalogID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
