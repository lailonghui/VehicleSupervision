...
func (r *mutationResolver) DeleteMuckTruckSaleOrder(ctx context.Context, where model.MuckTruckSaleOrderBoolExp) (*model.MuckTruckSaleOrderMutationResponse, error) {
	...
	var m = &model1.MuckTruckSaleOrder{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteMuckTruckSaleOrderByUnionPk(ctx context.Context, muckTruckSaleOrderID string) (*model1.MuckTruckSaleOrder, error) {
	var rs model1.MuckTruckSaleOrder
	m := &model1.MuckTruckSaleOrder{}
	tx := db.DB.Model(&model1.MuckTruckSaleOrder{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", muckTruckSaleOrderID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertMuckTruckSaleOrder(ctx context.Context, objects []*model.MuckTruckSaleOrderInsertInput) (*model.MuckTruckSaleOrderMutationResponse, error) {
	rs := make([]*model1.MuckTruckSaleOrder, 0)
	m := &model1.MuckTruckSaleOrder{}
	// 结构转换
...
func (r *mutationResolver) InsertMuckTruckSaleOrderOne(ctx context.Context, objects model.MuckTruckSaleOrderInsertInput) (*model1.MuckTruckSaleOrder, error) {
...
func (r *mutationResolver) UpdateMuckTruckSaleOrderByUnionPk(ctx context.Context, inc *model.MuckTruckSaleOrderIncInput, set *model.MuckTruckSaleOrderSetInput, muckTruckSaleOrderID string) (*model1.MuckTruckSaleOrder, error) {
	var rs model1.MuckTruckSaleOrder
	var m = &model1.MuckTruckSaleOrder{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", muckTruckSaleOrderID)
	qt := util.NewQueryTranslator(tx, &model1.MuckTruckSaleOrder{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) MuckTruckSaleOrderByUnionPk(ctx context.Context, muckTruckSaleOrderID string) (*model1.MuckTruckSaleOrder, error) {
	var rs model1.MuckTruckSaleOrder
	var m = &model1.MuckTruckSaleOrder{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := muckTruckSaleOrderID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", muckTruckSaleOrderID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
