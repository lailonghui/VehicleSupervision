...
func (r *mutationResolver) DeleteDriverIdentityInfoReport(ctx context.Context, where model.DriverIdentityInfoReportBoolExp) (*model.DriverIdentityInfoReportMutationResponse, error) {
	...
	var m = &model1.DriverIdentityInfoReport{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteDriverIdentityInfoReportByPk(ctx context.Context, id int64) (*model1.DriverIdentityInfoReport, error) {
	var rs model1.DriverIdentityInfoReport
	m := &model1.DriverIdentityInfoReport{}
	tx := db.DB.Model(m)
	// 查询记录
...
func (r *mutationResolver) DeleteDriverIdentityInfoReportByUnionPk(ctx context.Context, driverIdentityInfoReportID string) (*model1.DriverIdentityInfoReport, error) {
	var rs model1.DriverIdentityInfoReport
	m := &model1.DriverIdentityInfoReport{}
	tx := db.DB.Model(&model1.DriverIdentityInfoReport{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", driverIdentityInfoReportID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertDriverIdentityInfoReportOne(ctx context.Context, objects model.DriverIdentityInfoReportInsertInput) (*model1.DriverIdentityInfoReport, error) {
...
func (r *mutationResolver) UpdateDriverIdentityInfoReportByUnionPk(ctx context.Context, inc *model.DriverIdentityInfoReportIncInput, set *model.DriverIdentityInfoReportSetInput, driverIdentityInfoReportID string) (*model1.DriverIdentityInfoReport, error) {
	var rs model1.DriverIdentityInfoReport
	var m = &model1.DriverIdentityInfoReport{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", driverIdentityInfoReportID)
	qt := util.NewQueryTranslator(tx, &model1.DriverIdentityInfoReport{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) DriverIdentityInfoReportByUnionPk(ctx context.Context, driverIdentityInfoReportID string) (*model1.DriverIdentityInfoReport, error) {
	var rs model1.DriverIdentityInfoReport
	var m = &model1.DriverIdentityInfoReport{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := driverIdentityInfoReportID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", driverIdentityInfoReportID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
