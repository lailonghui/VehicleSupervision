...
func (r *mutationResolver) DeleteDriverFingerprint(ctx context.Context, where model.DriverFingerprintBoolExp) (*model.DriverFingerprintMutationResponse, error) {
	...
	var m = &model1.DriverFingerprint{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteDriverFingerprintByUnionPk(ctx context.Context, driverFingerprintID string) (*model1.DriverFingerprint, error) {
	var rs model1.DriverFingerprint
	m := &model1.DriverFingerprint{}
	tx := db.DB.Model(&model1.DriverFingerprint{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", driverFingerprintID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertDriverFingerprintOne(ctx context.Context, objects model.DriverFingerprintInsertInput) (*model1.DriverFingerprint, error) {
...
func (r *mutationResolver) UpdateDriverFingerprintByUnionPk(ctx context.Context, inc *model.DriverFingerprintIncInput, set *model.DriverFingerprintSetInput, driverFingerprintID string) (*model1.DriverFingerprint, error) {
	var rs model1.DriverFingerprint
	var m = &model1.DriverFingerprint{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", driverFingerprintID)
	qt := util.NewQueryTranslator(tx, &model1.DriverFingerprint{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) DriverFingerprintByUnionPk(ctx context.Context, driverFingerprintID string) (*model1.DriverFingerprint, error) {
	var rs model1.DriverFingerprint
	var m = &model1.DriverFingerprint{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := driverFingerprintID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", driverFingerprintID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
