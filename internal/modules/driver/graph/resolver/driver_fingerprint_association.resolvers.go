...
func (r *mutationResolver) DeleteDriverFingerprintAssociation(ctx context.Context, where model.DriverFingerprintAssociationBoolExp) (*model.DriverFingerprintAssociationMutationResponse, error) {
	...
	var m = &model1.DriverFingerprintAssociation{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteDriverFingerprintAssociationByPk(ctx context.Context, id int64) (*model1.DriverFingerprintAssociation, error) {
	var rs model1.DriverFingerprintAssociation
	m := &model1.DriverFingerprintAssociation{}
	tx := db.DB.Model(m)
	// 查询记录
...
func (r *mutationResolver) DeleteDriverFingerprintAssociationByUnionPk(ctx context.Context, driverFingerprintAssociationID string) (*model1.DriverFingerprintAssociation, error) {
	var rs model1.DriverFingerprintAssociation
	m := &model1.DriverFingerprintAssociation{}
	tx := db.DB.Model(&model1.DriverFingerprintAssociation{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", driverFingerprintAssociationID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertDriverFingerprintAssociationOne(ctx context.Context, objects model.DriverFingerprintAssociationInsertInput) (*model1.DriverFingerprintAssociation, error) {
...
func (r *mutationResolver) UpdateDriverFingerprintAssociationByUnionPk(ctx context.Context, inc *model.DriverFingerprintAssociationIncInput, set *model.DriverFingerprintAssociationSetInput, driverFingerprintAssociationID string) (*model1.DriverFingerprintAssociation, error) {
	var rs model1.DriverFingerprintAssociation
	var m = &model1.DriverFingerprintAssociation{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", driverFingerprintAssociationID)
	qt := util.NewQueryTranslator(tx, &model1.DriverFingerprintAssociation{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) DriverFingerprintAssociationByUnionPk(ctx context.Context, driverFingerprintAssociationID string) (*model1.DriverFingerprintAssociation, error) {
	var rs model1.DriverFingerprintAssociation
	var m = &model1.DriverFingerprintAssociation{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := driverFingerprintAssociationID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", driverFingerprintAssociationID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
