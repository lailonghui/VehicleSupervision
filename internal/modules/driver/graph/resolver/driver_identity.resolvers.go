...
func (r *mutationResolver) DeleteDriverIdentityByUnionPk(ctx context.Context, identityID string) (*model1.DriverIdentity, error) {
	var rs model1.DriverIdentity
	m := &model1.DriverIdentity{}
	tx := db.DB.Model(&model1.DriverIdentity{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", identityID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertDriverIdentityOne(ctx context.Context, objects model.DriverIdentityInsertInput) (*model1.DriverIdentity, error) {
...
func (r *mutationResolver) UpdateDriverIdentityByUnionPk(ctx context.Context, inc *model.DriverIdentityIncInput, set *model.DriverIdentitySetInput, identityID string) (*model1.DriverIdentity, error) {
	var rs model1.DriverIdentity
	var m = &model1.DriverIdentity{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", identityID)
	qt := util.NewQueryTranslator(tx, &model1.DriverIdentity{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) DriverIdentityByUnionPk(ctx context.Context, identityID string) (*model1.DriverIdentity, error) {
	var rs model1.DriverIdentity
	var m = &model1.DriverIdentity{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", identityID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
