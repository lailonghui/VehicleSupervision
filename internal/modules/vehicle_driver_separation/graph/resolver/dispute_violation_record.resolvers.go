...
func (r *mutationResolver) DeleteDisputeViolationRecordByPk(ctx context.Context, Id int64) (*model1.DisputeViolationRecord, error) {
	...
	var rs model1.DisputeViolationRecord
	tx := db.DB.Model(&model1.DisputeViolationRecord{})
	if len(preloads) > 0 {
		// 如果请求的字段不为空，则先查询一遍数据库
...
func (r *mutationResolver) InsertDisputeViolationRecordOne(ctx context.Context, object model.DisputeViolationRecordInsertInput) (*model1.DisputeViolationRecord, error) {
	rs := &model1.DisputeViolationRecord{}
