...
func (r *mutationResolver) DeleteDrivingLogInfoByUnionPk(ctx context.Context, drivingLogInfoID string) (*model1.DrivingLogInfo, error) {
	var rs model1.DrivingLogInfo
	m := &model1.DrivingLogInfo{}
	tx := db.DB.Model(&model1.DrivingLogInfo{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", drivingLogInfoID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertDrivingLogInfoOne(ctx context.Context, objects model.DrivingLogInfoInsertInput) (*model1.DrivingLogInfo, error) {
...
func (r *mutationResolver) UpdateDrivingLogInfoByUnionPk(ctx context.Context, inc *model.DrivingLogInfoIncInput, set *model.DrivingLogInfoSetInput, drivingLogInfoID string) (*model1.DrivingLogInfo, error) {
	var rs model1.DrivingLogInfo
	var m = &model1.DrivingLogInfo{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", drivingLogInfoID)
	qt := util.NewQueryTranslator(tx, &model1.DrivingLogInfo{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) DrivingLogInfoByUnionPk(ctx context.Context, drivingLogInfoID string) (*model1.DrivingLogInfo, error) {
	var rs model1.DrivingLogInfo
	var m = &model1.DrivingLogInfo{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := drivingLogInfoID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", drivingLogInfoID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
