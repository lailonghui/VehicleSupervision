...
func (r *mutationResolver) DeleteVehicleDriverBinding(ctx context.Context, where model.VehicleDriverBindingBoolExp) (*model.VehicleDriverBindingMutationResponse, error) {
	...
	var m = &model1.VehicleDriverBinding{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteVehicleDriverBindingByUnionPk(ctx context.Context, vehicleDriverBindingID string) (*model1.VehicleDriverBinding, error) {
	var rs model1.VehicleDriverBinding
	m := &model1.VehicleDriverBinding{}
	tx := db.DB.Model(&model1.VehicleDriverBinding{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", vehicleDriverBindingID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertVehicleDriverBindingOne(ctx context.Context, objects model.VehicleDriverBindingInsertInput) (*model1.VehicleDriverBinding, error) {
...
func (r *mutationResolver) UpdateVehicleDriverBindingByUnionPk(ctx context.Context, inc *model.VehicleDriverBindingIncInput, set *model.VehicleDriverBindingSetInput, vehicleDriverBindingID string) (*model1.VehicleDriverBinding, error) {
	var rs model1.VehicleDriverBinding
	var m = &model1.VehicleDriverBinding{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", vehicleDriverBindingID)
	qt := util.NewQueryTranslator(tx, &model1.VehicleDriverBinding{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) VehicleDriverBindingByUnionPk(ctx context.Context, vehicleDriverBindingID string) (*model1.VehicleDriverBinding, error) {
	var rs model1.VehicleDriverBinding
	var m = &model1.VehicleDriverBinding{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := vehicleDriverBindingID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", vehicleDriverBindingID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
