...
func (r *mutationResolver) DeleteCaseApprovalReviewCall(ctx context.Context, where model.CaseApprovalReviewCallBoolExp) (*model.CaseApprovalReviewCallMutationResponse, error) {
	...
	var m = &model1.CaseApprovalReviewCall{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteCaseApprovalReviewCallByPk(ctx context.Context, id int64) (*model1.CaseApprovalReviewCall, error) {
	var rs model1.CaseApprovalReviewCall
	m := &model1.CaseApprovalReviewCall{}
	tx := db.DB.Model(m)
	// 查询记录
...
func (r *mutationResolver) DeleteCaseApprovalReviewCallByUnionPk(ctx context.Context, caseApprovalReviewCallID string) (*model1.CaseApprovalReviewCall, error) {
	var rs model1.CaseApprovalReviewCall
	m := &model1.CaseApprovalReviewCall{}
	tx := db.DB.Model(&model1.CaseApprovalReviewCall{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", caseApprovalReviewCallID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertCaseApprovalReviewCallOne(ctx context.Context, objects model.CaseApprovalReviewCallInsertInput) (*model1.CaseApprovalReviewCall, error) {
...
func (r *mutationResolver) UpdateCaseApprovalReviewCallByUnionPk(ctx context.Context, inc *model.CaseApprovalReviewCallIncInput, set *model.CaseApprovalReviewCallSetInput, caseApprovalReviewCallID string) (*model1.CaseApprovalReviewCall, error) {
	var rs model1.CaseApprovalReviewCall
	var m = &model1.CaseApprovalReviewCall{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", caseApprovalReviewCallID)
	qt := util.NewQueryTranslator(tx, &model1.CaseApprovalReviewCall{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) CaseApprovalReviewCallByUnionPk(ctx context.Context, caseApprovalReviewCallID string) (*model1.CaseApprovalReviewCall, error) {
	var rs model1.CaseApprovalReviewCall
	var m = &model1.CaseApprovalReviewCall{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := caseApprovalReviewCallID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", caseApprovalReviewCallID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
