...
func (r *mutationResolver) DeleteCaseApprovalReviewOperation(ctx context.Context, where model.CaseApprovalReviewOperationBoolExp) (*model.CaseApprovalReviewOperationMutationResponse, error) {
	...
	var m = &model1.CaseApprovalReviewOperation{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteCaseApprovalReviewOperationByPk(ctx context.Context, id int64) (*model1.CaseApprovalReviewOperation, error) {
	var rs model1.CaseApprovalReviewOperation
	m := &model1.CaseApprovalReviewOperation{}
	tx := db.DB.Model(m)
	// 查询记录
...
func (r *mutationResolver) DeleteCaseApprovalReviewOperationByUnionPk(ctx context.Context, caseApprovalReviewOperationID string) (*model1.CaseApprovalReviewOperation, error) {
	var rs model1.CaseApprovalReviewOperation
	m := &model1.CaseApprovalReviewOperation{}
	tx := db.DB.Model(&model1.CaseApprovalReviewOperation{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", caseApprovalReviewOperationID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertCaseApprovalReviewOperationOne(ctx context.Context, objects model.CaseApprovalReviewOperationInsertInput) (*model1.CaseApprovalReviewOperation, error) {
...
func (r *mutationResolver) UpdateCaseApprovalReviewOperationByUnionPk(ctx context.Context, inc *model.CaseApprovalReviewOperationIncInput, set *model.CaseApprovalReviewOperationSetInput, caseApprovalReviewOperationID string) (*model1.CaseApprovalReviewOperation, error) {
	var rs model1.CaseApprovalReviewOperation
	var m = &model1.CaseApprovalReviewOperation{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", caseApprovalReviewOperationID)
	qt := util.NewQueryTranslator(tx, &model1.CaseApprovalReviewOperation{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) CaseApprovalReviewOperationByUnionPk(ctx context.Context, caseApprovalReviewOperationID string) (*model1.CaseApprovalReviewOperation, error) {
	var rs model1.CaseApprovalReviewOperation
	var m = &model1.CaseApprovalReviewOperation{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := caseApprovalReviewOperationID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", caseApprovalReviewOperationID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
