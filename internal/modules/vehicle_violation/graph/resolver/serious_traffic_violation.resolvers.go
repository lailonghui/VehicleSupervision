...
func (r *mutationResolver) DeleteSeriousTrafficViolation(ctx context.Context, where model.SeriousTrafficViolationBoolExp) (*model.SeriousTrafficViolationMutationResponse, error) {
	...
	var m = &model1.SeriousTrafficViolation{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteSeriousTrafficViolationByPk(ctx context.Context, id int64) (*model1.SeriousTrafficViolation, error) {
	var rs model1.SeriousTrafficViolation
	m := &model1.SeriousTrafficViolation{}
	tx := db.DB.Model(m)
	// 查询记录
...
func (r *mutationResolver) DeleteSeriousTrafficViolationByUnionPk(ctx context.Context, seriousTrafficViolationID string) (*model1.SeriousTrafficViolation, error) {
	var rs model1.SeriousTrafficViolation
	m := &model1.SeriousTrafficViolation{}
	tx := db.DB.Model(&model1.SeriousTrafficViolation{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", seriousTrafficViolationID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertSeriousTrafficViolation(ctx context.Context, objects []*model.SeriousTrafficViolationInsertInput) (*model.SeriousTrafficViolationMutationResponse, error) {
	rs := make([]*model1.SeriousTrafficViolation, 0)
	m := &model1.SeriousTrafficViolation{}
	// 结构转换
...
func (r *mutationResolver) InsertSeriousTrafficViolationOne(ctx context.Context, objects model.SeriousTrafficViolationInsertInput) (*model1.SeriousTrafficViolation, error) {
...
func (r *mutationResolver) UpdateSeriousTrafficViolationByUnionPk(ctx context.Context, inc *model.SeriousTrafficViolationIncInput, set *model.SeriousTrafficViolationSetInput, seriousTrafficViolationID string) (*model1.SeriousTrafficViolation, error) {
	var rs model1.SeriousTrafficViolation
	var m = &model1.SeriousTrafficViolation{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", seriousTrafficViolationID)
	qt := util.NewQueryTranslator(tx, &model1.SeriousTrafficViolation{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) SeriousTrafficViolationByUnionPk(ctx context.Context, seriousTrafficViolationID string) (*model1.SeriousTrafficViolation, error) {
	var rs model1.SeriousTrafficViolation
	var m = &model1.SeriousTrafficViolation{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := seriousTrafficViolationID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", seriousTrafficViolationID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
