...
func (r *mutationResolver) DeleteVioCodewfdm(ctx context.Context, where model.VioCodewfdmBoolExp) (*model.VioCodewfdmMutationResponse, error) {
	...
	var idList = make([]string, 0, amount)
	for i := 0; i < amount; i++ {
...
func (r *mutationResolver) DeleteVioCodewfdmByPk(ctx context.Context, xh string) (*model1.VioCodewfdm, error) {
	var rs model1.VioCodewfdm
	m := &model1.VioCodewfdm{}
	tx := db.DB.Model(m)
	// 查询记录
	tx = tx.Where(rs.PrimaryColumnName()+" = ?", xh).First(&rs)
	if err := tx.Error; err != nil {
...
func (r *mutationResolver) InsertVioCodewfdmOne(ctx context.Context, objects model.VioCodewfdmInsertInput) (*model1.VioCodewfdm, error) {
...
func (r *mutationResolver) UpdateVioCodewfdmByPk(ctx context.Context, inc *model.VioCodewfdmIncInput, set *model.VioCodewfdmSetInput, xh string) (*model1.VioCodewfdm, error) {
	var rs model1.VioCodewfdm
	var m = &model1.VioCodewfdm{}
	// 更新数据库
	tx := db.DB.Where(rs.PrimaryColumnName()+" = ?", xh)
...
func (r *queryResolver) VioCodewfdmByPk(ctx context.Context, xh string) (*model1.VioCodewfdm, error) {
