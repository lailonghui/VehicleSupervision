...
func (r *mutationResolver) DeleteDeductionReportByUnionPk(ctx context.Context, deductionReportID string) (*model1.DeductionReport, error) {
	var rs model1.DeductionReport
	m := &model1.DeductionReport{}
	tx := db.DB.Model(&model1.DeductionReport{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", deductionReportID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertDeductionReport(ctx context.Context, objects []*model.DeductionReportInsertInput) (*model.DeductionReportMutationResponse, error) {
	rs := make([]*model1.DeductionReport, 0)
	m := &model1.DeductionReport{}
	// 结构转换
...
func (r *mutationResolver) InsertDeductionReportOne(ctx context.Context, objects model.DeductionReportInsertInput) (*model1.DeductionReport, error) {
...
func (r *mutationResolver) UpdateDeductionReportByUnionPk(ctx context.Context, inc *model.DeductionReportIncInput, set *model.DeductionReportSetInput, deductionReportID string) (*model1.DeductionReport, error) {
	var rs model1.DeductionReport
	var m = &model1.DeductionReport{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", deductionReportID)
	qt := util.NewQueryTranslator(tx, &model1.DeductionReport{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) DeductionReportByUnionPk(ctx context.Context, deductionReportID string) (*model1.DeductionReport, error) {
	var rs model1.DeductionReport
	var m = &model1.DeductionReport{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := deductionReportID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", deductionReportID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
