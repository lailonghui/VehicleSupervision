...
func (r *mutationResolver) DeleteMuckTruckTestSituation(ctx context.Context, where model.MuckTruckTestSituationBoolExp) (*model.MuckTruckTestSituationMutationResponse, error) {
	...
	var m = &model1.MuckTruckTestSituation{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteMuckTruckTestSituationByPk(ctx context.Context, id int64) (*model1.MuckTruckTestSituation, error) {
	var rs model1.MuckTruckTestSituation
	m := &model1.MuckTruckTestSituation{}
	tx := db.DB.Model(m)
	// 查询记录
...
func (r *mutationResolver) DeleteMuckTruckTestSituationByUnionPk(ctx context.Context, muckTruckTestSituationID string) (*model1.MuckTruckTestSituation, error) {
	var rs model1.MuckTruckTestSituation
	m := &model1.MuckTruckTestSituation{}
	tx := db.DB.Model(&model1.MuckTruckTestSituation{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", muckTruckTestSituationID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertMuckTruckTestSituationOne(ctx context.Context, objects model.MuckTruckTestSituationInsertInput) (*model1.MuckTruckTestSituation, error) {
...
func (r *mutationResolver) UpdateMuckTruckTestSituationByUnionPk(ctx context.Context, inc *model.MuckTruckTestSituationIncInput, set *model.MuckTruckTestSituationSetInput, muckTruckTestSituationID string) (*model1.MuckTruckTestSituation, error) {
	var rs model1.MuckTruckTestSituation
	var m = &model1.MuckTruckTestSituation{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", muckTruckTestSituationID)
	qt := util.NewQueryTranslator(tx, &model1.MuckTruckTestSituation{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) MuckTruckTestSituationByUnionPk(ctx context.Context, muckTruckTestSituationID string) (*model1.MuckTruckTestSituation, error) {
	var rs model1.MuckTruckTestSituation
	var m = &model1.MuckTruckTestSituation{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := muckTruckTestSituationID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", muckTruckTestSituationID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
