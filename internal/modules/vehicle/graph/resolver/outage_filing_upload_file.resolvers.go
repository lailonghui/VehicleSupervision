...
func (r *mutationResolver) DeleteOutageFilingUploadFile(ctx context.Context, where model.OutageFilingUploadFileBoolExp) (*model.OutageFilingUploadFileMutationResponse, error) {
	...
	var m = &model1.OutageFilingUploadFile{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteOutageFilingUploadFileByPk(ctx context.Context, id int64) (*model1.OutageFilingUploadFile, error) {
	var rs model1.OutageFilingUploadFile
	m := &model1.OutageFilingUploadFile{}
	tx := db.DB.Model(m)
	// 查询记录
...
func (r *mutationResolver) DeleteOutageFilingUploadFileByUnionPk(ctx context.Context, outageFilingUploadFileID string) (*model1.OutageFilingUploadFile, error) {
	var rs model1.OutageFilingUploadFile
	m := &model1.OutageFilingUploadFile{}
	tx := db.DB.Model(&model1.OutageFilingUploadFile{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", outageFilingUploadFileID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertOutageFilingUploadFile(ctx context.Context, objects []*model.OutageFilingUploadFileInsertInput) (*model.OutageFilingUploadFileMutationResponse, error) {
	rs := make([]*model1.OutageFilingUploadFile, 0)
	m := &model1.OutageFilingUploadFile{}
	// 结构转换
...
func (r *mutationResolver) InsertOutageFilingUploadFileOne(ctx context.Context, objects model.OutageFilingUploadFileInsertInput) (*model1.OutageFilingUploadFile, error) {
...
func (r *mutationResolver) UpdateOutageFilingUploadFileByUnionPk(ctx context.Context, inc *model.OutageFilingUploadFileIncInput, set *model.OutageFilingUploadFileSetInput, outageFilingUploadFileID string) (*model1.OutageFilingUploadFile, error) {
	var rs model1.OutageFilingUploadFile
	var m = &model1.OutageFilingUploadFile{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", outageFilingUploadFileID)
	qt := util.NewQueryTranslator(tx, &model1.OutageFilingUploadFile{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) OutageFilingUploadFileByUnionPk(ctx context.Context, outageFilingUploadFileID string) (*model1.OutageFilingUploadFile, error) {
	var rs model1.OutageFilingUploadFile
	var m = &model1.OutageFilingUploadFile{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := outageFilingUploadFileID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", outageFilingUploadFileID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
