...
func (r *mutationResolver) DeleteMuckTruckPreviewNumber(ctx context.Context, where model.MuckTruckPreviewNumberBoolExp) (*model.MuckTruckPreviewNumberMutationResponse, error) {
	...
	var m = &model1.MuckTruckPreviewNumber{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteMuckTruckPreviewNumberByPk(ctx context.Context, id int64) (*model1.MuckTruckPreviewNumber, error) {
	var rs model1.MuckTruckPreviewNumber
	m := &model1.MuckTruckPreviewNumber{}
	tx := db.DB.Model(m)
	// 查询记录
...
func (r *mutationResolver) DeleteMuckTruckPreviewNumberByUnionPk(ctx context.Context, muckTruckPreviewNumberID string) (*model1.MuckTruckPreviewNumber, error) {
	var rs model1.MuckTruckPreviewNumber
	m := &model1.MuckTruckPreviewNumber{}
	tx := db.DB.Model(&model1.MuckTruckPreviewNumber{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", muckTruckPreviewNumberID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertMuckTruckPreviewNumber(ctx context.Context, objects []*model.MuckTruckPreviewNumberInsertInput) (*model.MuckTruckPreviewNumberMutationResponse, error) {
	rs := make([]*model1.MuckTruckPreviewNumber, 0)
	m := &model1.MuckTruckPreviewNumber{}
	// 结构转换
...
func (r *mutationResolver) InsertMuckTruckPreviewNumberOne(ctx context.Context, objects model.MuckTruckPreviewNumberInsertInput) (*model1.MuckTruckPreviewNumber, error) {
...
func (r *mutationResolver) UpdateMuckTruckPreviewNumberByUnionPk(ctx context.Context, inc *model.MuckTruckPreviewNumberIncInput, set *model.MuckTruckPreviewNumberSetInput, muckTruckPreviewNumberID string) (*model1.MuckTruckPreviewNumber, error) {
	var rs model1.MuckTruckPreviewNumber
	var m = &model1.MuckTruckPreviewNumber{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", muckTruckPreviewNumberID)
	qt := util.NewQueryTranslator(tx, &model1.MuckTruckPreviewNumber{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) MuckTruckPreviewNumberByUnionPk(ctx context.Context, muckTruckPreviewNumberID string) (*model1.MuckTruckPreviewNumber, error) {
	var rs model1.MuckTruckPreviewNumber
	var m = &model1.MuckTruckPreviewNumber{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := muckTruckPreviewNumberID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", muckTruckPreviewNumberID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
