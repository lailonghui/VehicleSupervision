...
func (r *mutationResolver) DeleteVehicleExitCatalogReview(ctx context.Context, where model.VehicleExitCatalogReviewBoolExp) (*model.VehicleExitCatalogReviewMutationResponse, error) {
	...
	var m = &model1.VehicleExitCatalogReview{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteVehicleExitCatalogReviewByPk(ctx context.Context, id int64) (*model1.VehicleExitCatalogReview, error) {
	var rs model1.VehicleExitCatalogReview
	m := &model1.VehicleExitCatalogReview{}
	tx := db.DB.Model(m)
	// 查询记录
...
func (r *mutationResolver) DeleteVehicleExitCatalogReviewByUnionPk(ctx context.Context, vehicleExitCatalogReviewID string) (*model1.VehicleExitCatalogReview, error) {
	var rs model1.VehicleExitCatalogReview
	m := &model1.VehicleExitCatalogReview{}
	tx := db.DB.Model(&model1.VehicleExitCatalogReview{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", vehicleExitCatalogReviewID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertVehicleExitCatalogReview(ctx context.Context, objects []*model.VehicleExitCatalogReviewInsertInput) (*model.VehicleExitCatalogReviewMutationResponse, error) {
	rs := make([]*model1.VehicleExitCatalogReview, 0)
	m := &model1.VehicleExitCatalogReview{}
	// 结构转换
...
func (r *mutationResolver) InsertVehicleExitCatalogReviewOne(ctx context.Context, objects model.VehicleExitCatalogReviewInsertInput) (*model1.VehicleExitCatalogReview, error) {
...
func (r *mutationResolver) UpdateVehicleExitCatalogReviewByUnionPk(ctx context.Context, inc *model.VehicleExitCatalogReviewIncInput, set *model.VehicleExitCatalogReviewSetInput, vehicleExitCatalogReviewID string) (*model1.VehicleExitCatalogReview, error) {
	var rs model1.VehicleExitCatalogReview
	var m = &model1.VehicleExitCatalogReview{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", vehicleExitCatalogReviewID)
	qt := util.NewQueryTranslator(tx, &model1.VehicleExitCatalogReview{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) VehicleExitCatalogReviewByUnionPk(ctx context.Context, vehicleExitCatalogReviewID string) (*model1.VehicleExitCatalogReview, error) {
	var rs model1.VehicleExitCatalogReview
	var m = &model1.VehicleExitCatalogReview{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := vehicleExitCatalogReviewID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", vehicleExitCatalogReviewID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
