...
func (r *mutationResolver) DeleteVehicleEnterpriseChangeLog(ctx context.Context, where model.VehicleEnterpriseChangeLogBoolExp) (*model.VehicleEnterpriseChangeLogMutationResponse, error) {
	...
	var m = &model1.VehicleEnterpriseChangeLog{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteVehicleEnterpriseChangeLogByPk(ctx context.Context, id int64) (*model1.VehicleEnterpriseChangeLog, error) {
	var rs model1.VehicleEnterpriseChangeLog
	m := &model1.VehicleEnterpriseChangeLog{}
	tx := db.DB.Model(m)
	// 查询记录
...
func (r *mutationResolver) DeleteVehicleEnterpriseChangeLogByUnionPk(ctx context.Context, vehicleEnterpriseChangeLogID string) (*model1.VehicleEnterpriseChangeLog, error) {
	var rs model1.VehicleEnterpriseChangeLog
	m := &model1.VehicleEnterpriseChangeLog{}
	tx := db.DB.Model(&model1.VehicleEnterpriseChangeLog{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", vehicleEnterpriseChangeLogID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertVehicleEnterpriseChangeLog(ctx context.Context, objects []*model.VehicleEnterpriseChangeLogInsertInput) (*model.VehicleEnterpriseChangeLogMutationResponse, error) {
	rs := make([]*model1.VehicleEnterpriseChangeLog, 0)
	m := &model1.VehicleEnterpriseChangeLog{}
	// 结构转换
...
func (r *mutationResolver) InsertVehicleEnterpriseChangeLogOne(ctx context.Context, objects model.VehicleEnterpriseChangeLogInsertInput) (*model1.VehicleEnterpriseChangeLog, error) {
...
func (r *mutationResolver) UpdateVehicleEnterpriseChangeLogByUnionPk(ctx context.Context, inc *model.VehicleEnterpriseChangeLogIncInput, set *model.VehicleEnterpriseChangeLogSetInput, vehicleEnterpriseChangeLogID string) (*model1.VehicleEnterpriseChangeLog, error) {
	var rs model1.VehicleEnterpriseChangeLog
	var m = &model1.VehicleEnterpriseChangeLog{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", vehicleEnterpriseChangeLogID)
	qt := util.NewQueryTranslator(tx, &model1.VehicleEnterpriseChangeLog{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) VehicleEnterpriseChangeLogByUnionPk(ctx context.Context, vehicleEnterpriseChangeLogID string) (*model1.VehicleEnterpriseChangeLog, error) {
	var rs model1.VehicleEnterpriseChangeLog
	var m = &model1.VehicleEnterpriseChangeLog{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := vehicleEnterpriseChangeLogID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", vehicleEnterpriseChangeLogID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
