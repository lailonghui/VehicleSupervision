...
func (r *mutationResolver) DeleteVehicleEnterpriseChangeReview(ctx context.Context, where model.VehicleEnterpriseChangeReviewBoolExp) (*model.VehicleEnterpriseChangeReviewMutationResponse, error) {
	...
	var m = &model1.VehicleEnterpriseChangeReview{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteVehicleEnterpriseChangeReviewByPk(ctx context.Context, id int64) (*model1.VehicleEnterpriseChangeReview, error) {
	var rs model1.VehicleEnterpriseChangeReview
	m := &model1.VehicleEnterpriseChangeReview{}
	tx := db.DB.Model(m)
	// 查询记录
...
func (r *mutationResolver) DeleteVehicleEnterpriseChangeReviewByUnionPk(ctx context.Context, vehicleEnterpriseChangeReviewID string) (*model1.VehicleEnterpriseChangeReview, error) {
	var rs model1.VehicleEnterpriseChangeReview
	m := &model1.VehicleEnterpriseChangeReview{}
	tx := db.DB.Model(&model1.VehicleEnterpriseChangeReview{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", vehicleEnterpriseChangeReviewID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertVehicleEnterpriseChangeReview(ctx context.Context, objects []*model.VehicleEnterpriseChangeReviewInsertInput) (*model.VehicleEnterpriseChangeReviewMutationResponse, error) {
	rs := make([]*model1.VehicleEnterpriseChangeReview, 0)
	m := &model1.VehicleEnterpriseChangeReview{}
	// 结构转换
...
func (r *mutationResolver) InsertVehicleEnterpriseChangeReviewOne(ctx context.Context, objects model.VehicleEnterpriseChangeReviewInsertInput) (*model1.VehicleEnterpriseChangeReview, error) {
...
func (r *mutationResolver) UpdateVehicleEnterpriseChangeReviewByUnionPk(ctx context.Context, inc *model.VehicleEnterpriseChangeReviewIncInput, set *model.VehicleEnterpriseChangeReviewSetInput, vehicleEnterpriseChangeReviewID string) (*model1.VehicleEnterpriseChangeReview, error) {
	var rs model1.VehicleEnterpriseChangeReview
	var m = &model1.VehicleEnterpriseChangeReview{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", vehicleEnterpriseChangeReviewID)
	qt := util.NewQueryTranslator(tx, &model1.VehicleEnterpriseChangeReview{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) VehicleEnterpriseChangeReviewByUnionPk(ctx context.Context, vehicleEnterpriseChangeReviewID string) (*model1.VehicleEnterpriseChangeReview, error) {
	var rs model1.VehicleEnterpriseChangeReview
	var m = &model1.VehicleEnterpriseChangeReview{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := vehicleEnterpriseChangeReviewID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", vehicleEnterpriseChangeReviewID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
