...
func (r *mutationResolver) DeleteVehicleStateLatest(ctx context.Context, where model.VehicleStateLatestBoolExp) (*model.VehicleStateLatestMutationResponse, error) {
	...
	var m = &model1.VehicleStateLatest{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteVehicleStateLatestByUnionPk(ctx context.Context, vehicleStateLatestID string) (*model1.VehicleStateLatest, error) {
	var rs model1.VehicleStateLatest
	m := &model1.VehicleStateLatest{}
	tx := db.DB.Model(&model1.VehicleStateLatest{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", vehicleStateLatestID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertVehicleStateLatestOne(ctx context.Context, objects model.VehicleStateLatestInsertInput) (*model1.VehicleStateLatest, error) {
...
func (r *mutationResolver) UpdateVehicleStateLatestByUnionPk(ctx context.Context, inc *model.VehicleStateLatestIncInput, set *model.VehicleStateLatestSetInput, vehicleStateLatestID string) (*model1.VehicleStateLatest, error) {
	var rs model1.VehicleStateLatest
	var m = &model1.VehicleStateLatest{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", vehicleStateLatestID)
	qt := util.NewQueryTranslator(tx, &model1.VehicleStateLatest{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) VehicleStateLatestByUnionPk(ctx context.Context, vehicleStateLatestID string) (*model1.VehicleStateLatest, error) {
	var rs model1.VehicleStateLatest
	var m = &model1.VehicleStateLatest{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := vehicleStateLatestID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", vehicleStateLatestID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
