...
func (r *mutationResolver) DeleteOutageRegistration(ctx context.Context, where model.OutageRegistrationBoolExp) (*model.OutageRegistrationMutationResponse, error) {
	...
	var m = &model1.OutageRegistration{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteOutageRegistrationByUnionPk(ctx context.Context, outageRegistrationID string) (*model1.OutageRegistration, error) {
	var rs model1.OutageRegistration
	m := &model1.OutageRegistration{}
	tx := db.DB.Model(&model1.OutageRegistration{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", outageRegistrationID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertOutageRegistrationOne(ctx context.Context, objects model.OutageRegistrationInsertInput) (*model1.OutageRegistration, error) {
...
func (r *mutationResolver) UpdateOutageRegistrationByUnionPk(ctx context.Context, inc *model.OutageRegistrationIncInput, set *model.OutageRegistrationSetInput, outageRegistrationID string) (*model1.OutageRegistration, error) {
	var rs model1.OutageRegistration
	var m = &model1.OutageRegistration{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", outageRegistrationID)
	qt := util.NewQueryTranslator(tx, &model1.OutageRegistration{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) OutageRegistrationByUnionPk(ctx context.Context, outageRegistrationID string) (*model1.OutageRegistration, error) {
	var rs model1.OutageRegistration
	var m = &model1.OutageRegistration{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := outageRegistrationID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", outageRegistrationID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
