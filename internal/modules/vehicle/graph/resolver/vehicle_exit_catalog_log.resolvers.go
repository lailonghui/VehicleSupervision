...
func (r *mutationResolver) DeleteVehicleExitCatalogLog(ctx context.Context, where model.VehicleExitCatalogLogBoolExp) (*model.VehicleExitCatalogLogMutationResponse, error) {
	...
	var m = &model1.VehicleExitCatalogLog{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteVehicleExitCatalogLogByPk(ctx context.Context, id int64) (*model1.VehicleExitCatalogLog, error) {
	var rs model1.VehicleExitCatalogLog
	m := &model1.VehicleExitCatalogLog{}
	tx := db.DB.Model(m)
	// 查询记录
...
func (r *mutationResolver) DeleteVehicleExitCatalogLogByUnionPk(ctx context.Context, vehicleExitCatalogLogID string) (*model1.VehicleExitCatalogLog, error) {
	var rs model1.VehicleExitCatalogLog
	m := &model1.VehicleExitCatalogLog{}
	tx := db.DB.Model(&model1.VehicleExitCatalogLog{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", vehicleExitCatalogLogID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertVehicleExitCatalogLogOne(ctx context.Context, objects model.VehicleExitCatalogLogInsertInput) (*model1.VehicleExitCatalogLog, error) {
...
func (r *mutationResolver) UpdateVehicleExitCatalogLogByUnionPk(ctx context.Context, inc *model.VehicleExitCatalogLogIncInput, set *model.VehicleExitCatalogLogSetInput, vehicleExitCatalogLogID string) (*model1.VehicleExitCatalogLog, error) {
	var rs model1.VehicleExitCatalogLog
	var m = &model1.VehicleExitCatalogLog{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", vehicleExitCatalogLogID)
	qt := util.NewQueryTranslator(tx, &model1.VehicleExitCatalogLog{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) VehicleExitCatalogLogByUnionPk(ctx context.Context, vehicleExitCatalogLogID string) (*model1.VehicleExitCatalogLog, error) {
	var rs model1.VehicleExitCatalogLog
	var m = &model1.VehicleExitCatalogLog{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := vehicleExitCatalogLogID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", vehicleExitCatalogLogID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
