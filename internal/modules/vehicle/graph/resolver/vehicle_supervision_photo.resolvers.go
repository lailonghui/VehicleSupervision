...
func (r *mutationResolver) DeleteVehicleSupervisionPhoto(ctx context.Context, where model.VehicleSupervisionPhotoBoolExp) (*model.VehicleSupervisionPhotoMutationResponse, error) {
	...
	var m = &model1.VehicleSupervisionPhoto{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteVehicleSupervisionPhotoByPk(ctx context.Context, id int64) (*model1.VehicleSupervisionPhoto, error) {
	var rs model1.VehicleSupervisionPhoto
	m := &model1.VehicleSupervisionPhoto{}
	tx := db.DB.Model(m)
	// 查询记录
...
func (r *mutationResolver) DeleteVehicleSupervisionPhotoByUnionPk(ctx context.Context, supervisionPhotoID string) (*model1.VehicleSupervisionPhoto, error) {
	var rs model1.VehicleSupervisionPhoto
	m := &model1.VehicleSupervisionPhoto{}
	tx := db.DB.Model(&model1.VehicleSupervisionPhoto{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", supervisionPhotoID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertVehicleSupervisionPhotoOne(ctx context.Context, objects model.VehicleSupervisionPhotoInsertInput) (*model1.VehicleSupervisionPhoto, error) {
...
func (r *mutationResolver) UpdateVehicleSupervisionPhotoByUnionPk(ctx context.Context, inc *model.VehicleSupervisionPhotoIncInput, set *model.VehicleSupervisionPhotoSetInput, supervisionPhotoID string) (*model1.VehicleSupervisionPhoto, error) {
	var rs model1.VehicleSupervisionPhoto
	var m = &model1.VehicleSupervisionPhoto{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", supervisionPhotoID)
	qt := util.NewQueryTranslator(tx, &model1.VehicleSupervisionPhoto{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) VehicleSupervisionPhotoByUnionPk(ctx context.Context, supervisionPhotoID string) (*model1.VehicleSupervisionPhoto, error) {
	var rs model1.VehicleSupervisionPhoto
	var m = &model1.VehicleSupervisionPhoto{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := supervisionPhotoID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", supervisionPhotoID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
