...
func (r *mutationResolver) DeleteVehicleReserveHistoryRecord(ctx context.Context, where model.VehicleReserveHistoryRecordBoolExp) (*model.VehicleReserveHistoryRecordMutationResponse, error) {
	...
	var m = &model1.VehicleReserveHistoryRecord{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteVehicleReserveHistoryRecordByPk(ctx context.Context, id int64) (*model1.VehicleReserveHistoryRecord, error) {
	var rs model1.VehicleReserveHistoryRecord
	m := &model1.VehicleReserveHistoryRecord{}
	tx := db.DB.Model(m)
	// 查询记录
...
func (r *mutationResolver) DeleteVehicleReserveHistoryRecordByUnionPk(ctx context.Context, vehicleReserveHistoryRecordID string) (*model1.VehicleReserveHistoryRecord, error) {
	var rs model1.VehicleReserveHistoryRecord
	m := &model1.VehicleReserveHistoryRecord{}
	tx := db.DB.Model(&model1.VehicleReserveHistoryRecord{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", vehicleReserveHistoryRecordID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertVehicleReserveHistoryRecord(ctx context.Context, objects []*model.VehicleReserveHistoryRecordInsertInput) (*model.VehicleReserveHistoryRecordMutationResponse, error) {
	rs := make([]*model1.VehicleReserveHistoryRecord, 0)
	m := &model1.VehicleReserveHistoryRecord{}
	// 结构转换
...
func (r *mutationResolver) InsertVehicleReserveHistoryRecordOne(ctx context.Context, objects model.VehicleReserveHistoryRecordInsertInput) (*model1.VehicleReserveHistoryRecord, error) {
...
func (r *mutationResolver) UpdateVehicleReserveHistoryRecordByUnionPk(ctx context.Context, inc *model.VehicleReserveHistoryRecordIncInput, set *model.VehicleReserveHistoryRecordSetInput, vehicleReserveHistoryRecordID string) (*model1.VehicleReserveHistoryRecord, error) {
	var rs model1.VehicleReserveHistoryRecord
	var m = &model1.VehicleReserveHistoryRecord{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", vehicleReserveHistoryRecordID)
	qt := util.NewQueryTranslator(tx, &model1.VehicleReserveHistoryRecord{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) VehicleReserveHistoryRecordByUnionPk(ctx context.Context, vehicleReserveHistoryRecordID string) (*model1.VehicleReserveHistoryRecord, error) {
	var rs model1.VehicleReserveHistoryRecord
	var m = &model1.VehicleReserveHistoryRecord{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := vehicleReserveHistoryRecordID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", vehicleReserveHistoryRecordID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
