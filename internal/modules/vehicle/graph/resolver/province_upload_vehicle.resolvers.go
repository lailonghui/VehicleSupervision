...
func (r *mutationResolver) DeleteProvinceUploadVehicle(ctx context.Context, where model.ProvinceUploadVehicleBoolExp) (*model.ProvinceUploadVehicleMutationResponse, error) {
	...
	var m = &model1.ProvinceUploadVehicle{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteProvinceUploadVehicleByPk(ctx context.Context, id int64) (*model1.ProvinceUploadVehicle, error) {
	var rs model1.ProvinceUploadVehicle
	m := &model1.ProvinceUploadVehicle{}
	tx := db.DB.Model(m)
	// 查询记录
...
func (r *mutationResolver) DeleteProvinceUploadVehicleByUnionPk(ctx context.Context, provinceUploadVehicleID string) (*model1.ProvinceUploadVehicle, error) {
	var rs model1.ProvinceUploadVehicle
	m := &model1.ProvinceUploadVehicle{}
	tx := db.DB.Model(&model1.ProvinceUploadVehicle{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", provinceUploadVehicleID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertProvinceUploadVehicleOne(ctx context.Context, objects model.ProvinceUploadVehicleInsertInput) (*model1.ProvinceUploadVehicle, error) {
...
func (r *mutationResolver) UpdateProvinceUploadVehicleByUnionPk(ctx context.Context, inc *model.ProvinceUploadVehicleIncInput, set *model.ProvinceUploadVehicleSetInput, provinceUploadVehicleID string) (*model1.ProvinceUploadVehicle, error) {
	var rs model1.ProvinceUploadVehicle
	var m = &model1.ProvinceUploadVehicle{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", provinceUploadVehicleID)
	qt := util.NewQueryTranslator(tx, &model1.ProvinceUploadVehicle{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) ProvinceUploadVehicleByUnionPk(ctx context.Context, provinceUploadVehicleID string) (*model1.ProvinceUploadVehicle, error) {
	var rs model1.ProvinceUploadVehicle
	var m = &model1.ProvinceUploadVehicle{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := provinceUploadVehicleID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", provinceUploadVehicleID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
