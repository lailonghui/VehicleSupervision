...
func (r *mutationResolver) DeleteJjVehicleByUnionPk(ctx context.Context, vehicleID string) (*model1.JjVehicle, error) {
	var rs model1.JjVehicle
	m := &model1.JjVehicle{}
	tx := db.DB.Model(&model1.JjVehicle{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", vehicleID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertJjVehicleOne(ctx context.Context, objects model.JjVehicleInsertInput) (*model1.JjVehicle, error) {
	rs := &model1.JjVehicle{}
	m := &model1.JjVehicle{}
	// 插入记录
...
func (r *mutationResolver) UpdateJjVehicleByUnionPk(ctx context.Context, inc *model.JjVehicleIncInput, set *model.JjVehicleSetInput, vehicleID string) (*model1.JjVehicle, error) {
	var rs model1.JjVehicle
	var m = &model1.JjVehicle{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", vehicleID)
	qt := util.NewQueryTranslator(tx, &model1.JjVehicle{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) JjVehicleByUnionPk(ctx context.Context, vehicleID string) (*model1.JjVehicle, error) {
	var rs model1.JjVehicle
	var m = &model1.JjVehicle{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", vehicleID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
