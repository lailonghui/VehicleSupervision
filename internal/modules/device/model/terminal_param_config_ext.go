...
// 新建主键dataloader
func (t *TerminalParamConfigPkLoader) NewLoader(ctx context.Context) *TerminalParamConfigPkLoader {
	return &TerminalParamConfigPkLoader{
		wait:     2 * time.Millisecond,
		maxBatch: 100,
		fetch: func(keys []string) ([]*TerminalParamConfig, []error) {
			var rs []*TerminalParamConfig = make([]*TerminalParamConfig, len(keys))
			...
			if middle.IsEnableGqlCache(ctx) {
				...
				for i := range keys {
					...
					go func(i int) {
						...
						// 如果启用缓存，则从缓存中查询数据
						var entity TerminalParamConfig
						cacheKey := keys[i]
						if cacheErr == nil {
...
// 新建联合主键dataloader
func (t *TerminalParamConfigUnionPkLoader) NewLoader(ctx context.Context) *TerminalParamConfigUnionPkLoader {
