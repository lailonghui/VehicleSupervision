...
// 新建主键dataloader
func (t *TerminalTypesPkLoader) NewLoader(ctx context.Context) *TerminalTypesPkLoader {
	return &TerminalTypesPkLoader{
		wait:     2 * time.Millisecond,
		maxBatch: 100,
		fetch: func(keys []string) ([]*TerminalTypes, []error) {
			var rs []*TerminalTypes = make([]*TerminalTypes, len(keys))
			var m TerminalTypes
			wg := sync.WaitGroup{}
