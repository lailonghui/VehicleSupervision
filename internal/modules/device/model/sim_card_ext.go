...
// 新建主键dataloader
func (t *SimCardPkLoader) NewLoader(ctx context.Context) *SimCardPkLoader {
	return &SimCardPkLoader{
		wait:     2 * time.Millisecond,
		maxBatch: 100,
		fetch: func(keys []string) ([]*SimCard, []error) {
			var rs []*SimCard = make([]*SimCard, len(keys))
			var m SimCard
			wg := sync.WaitGroup{}
...
// 新建联合主键dataloader
func (t *SimCardUnionPkLoader) NewLoader(ctx context.Context) *SimCardUnionPkLoader {
