...
// 新建主键dataloader
func (t *FingerprintPkLoader) NewLoader(ctx context.Context) *FingerprintPkLoader {
	return &FingerprintPkLoader{
		wait:     2 * time.Millisecond,
		maxBatch: 100,
		fetch: func(keys []string) ([]*Fingerprint, []error) {
			var rs []*Fingerprint = make([]*Fingerprint, len(keys))
			var m Fingerprint
			wg := sync.WaitGroup{}
...
// 新建联合主键dataloader
func (t *FingerprintUnionPkLoader) NewLoader(ctx context.Context) *FingerprintUnionPkLoader {
