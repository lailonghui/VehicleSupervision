...
// 新建主键dataloader
func (t *TerminalCheckPkLoader) NewLoader(ctx context.Context) *TerminalCheckPkLoader {
	return &TerminalCheckPkLoader{
		wait:     2 * time.Millisecond,
		maxBatch: 100,
		fetch: func(keys []string) ([]*TerminalCheck, []error) {
			var rs []*TerminalCheck = make([]*TerminalCheck, len(keys))
			var m TerminalCheck
			wg := sync.WaitGroup{}
...
// 新建联合主键dataloader
func (t *TerminalCheckUnionPkLoader) NewLoader(ctx context.Context) *TerminalCheckUnionPkLoader {
