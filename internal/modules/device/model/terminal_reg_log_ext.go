...
// 新建主键dataloader
func (t *TerminalRegLogPkLoader) NewLoader(ctx context.Context) *TerminalRegLogPkLoader {
	return &TerminalRegLogPkLoader{
		wait:     2 * time.Millisecond,
		maxBatch: 100,
		fetch: func(keys []string) ([]*TerminalRegLog, []error) {
			var rs []*TerminalRegLog = make([]*TerminalRegLog, len(keys))
