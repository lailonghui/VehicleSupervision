...
// 新建主键dataloader
func (t *TerminalProvePkLoader) NewLoader(ctx context.Context) *TerminalProvePkLoader {
	return &TerminalProvePkLoader{
		wait:     2 * time.Millisecond,
		maxBatch: 100,
		fetch: func(keys []string) ([]*TerminalProve, []error) {
			var rs []*TerminalProve = make([]*TerminalProve, len(keys))
			var m TerminalProve
			wg := sync.WaitGroup{}
...
// 新建联合主键dataloader
func (t *TerminalProveUnionPkLoader) NewLoader(ctx context.Context) *TerminalProveUnionPkLoader {
