...
func (r *mutationResolver) DeleteTerminalByUnionPk(ctx context.Context, terminalID string) (*model1.Terminal, error) {
	var rs model1.Terminal
	m := &model1.Terminal{}
	tx := db.DB.Model(&model1.Terminal{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", terminalID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertTerminalOne(ctx context.Context, objects model.TerminalInsertInput) (*model1.Terminal, error) {
	rs := &model1.Terminal{}
	m := &model1.Terminal{}
	// 插入记录
...
func (r *mutationResolver) UpdateTerminalByPk(ctx context.Context, inc *model.TerminalIncInput, set *model.TerminalSetInput, id int64) (*model1.Terminal, error) {
	var rs model1.Terminal
	var m = &model1.Terminal{}
	// 更新数据库
...
func (r *mutationResolver) UpdateTerminalByUnionPk(ctx context.Context, inc *model.TerminalIncInput, set *model.TerminalSetInput, terminalID string) (*model1.Terminal, error) {
	var rs model1.Terminal
	var m = &model1.Terminal{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", terminalID)
	qt := util.NewQueryTranslator(tx, &model1.Terminal{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) TerminalByUnionPk(ctx context.Context, terminalID string) (*model1.Terminal, error) {
	var rs model1.Terminal
	var m = &model1.Terminal{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", terminalID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
					_ = cacheAspect.SetNotExistUnionPkQueryCache(ctx, cacheKey, model1.Terminal{})
				}
				return &rs, nil
			}
			return nil, err
