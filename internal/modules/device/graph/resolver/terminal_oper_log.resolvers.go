...
func (r *mutationResolver) DeleteTerminalOperLogByUnionPk(ctx context.Context, logID string) (*model1.TerminalOperLog, error) {
	var rs model1.TerminalOperLog
	m := &model1.TerminalOperLog{}
...
func (r *mutationResolver) InsertTerminalOperLogOne(ctx context.Context, objects model.TerminalOperLogInsertInput) (*model1.TerminalOperLog, error) {
...
func (r *mutationResolver) UpdateTerminalOperLogByUnionPk(ctx context.Context, inc *model.TerminalOperLogIncInput, set *model.TerminalOperLogSetInput, logID string) (*model1.TerminalOperLog, error) {
	var rs model1.TerminalOperLog
	var m = &model1.TerminalOperLog{}
	// 更新数据库
...
func (r *queryResolver) TerminalOperLogByUnionPk(ctx context.Context, logID string) (*model1.TerminalOperLog, error) {
	var rs model1.TerminalOperLog
	var m = &model1.TerminalOperLog{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		...
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
					_ = cacheAspect.SetNotExistUnionPkQueryCache(ctx, cacheKey, model1.TerminalOperLog{})
				}
				return &rs, nil
			}
			return nil, err
