...
func (r *mutationResolver) DeleteTerminalChangeByUnionPk(ctx context.Context, changeID string) (*model1.TerminalChange, error) {
	var rs model1.TerminalChange
	m := &model1.TerminalChange{}
	tx := db.DB.Model(&model1.TerminalChange{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", changeID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertTerminalChangeOne(ctx context.Context, objects model.TerminalChangeInsertInput) (*model1.TerminalChange, error) {
...
func (r *mutationResolver) UpdateTerminalChangeByUnionPk(ctx context.Context, inc *model.TerminalChangeIncInput, set *model.TerminalChangeSetInput, changeID string) (*model1.TerminalChange, error) {
	var rs model1.TerminalChange
	var m = &model1.TerminalChange{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", changeID)
	qt := util.NewQueryTranslator(tx, &model1.TerminalChange{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) TerminalChangeByUnionPk(ctx context.Context, changeID string) (*model1.TerminalChange, error) {
	var rs model1.TerminalChange
	var m = &model1.TerminalChange{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", changeID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
					_ = cacheAspect.SetNotExistUnionPkQueryCache(ctx, cacheKey, model1.TerminalChange{})
				}
				return &rs, nil
			}
			return nil, err
