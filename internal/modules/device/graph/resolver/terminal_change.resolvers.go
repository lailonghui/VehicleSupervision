...
func (r *mutationResolver) DeleteTerminalChangeByUnionPk(ctx context.Context, unionId string) (*model1.TerminalChange, error) {
	preloads := util.GetPreloads(ctx)
	var rs model1.TerminalChange
	tx := db.DB.Model(&model1.TerminalChange{})
	if len(preloads) > 0 {
...
func (r *mutationResolver) UpdateTerminalChangeByUnionPk(ctx context.Context, inc *model.TerminalChangeIncInput, set *model.TerminalChangeSetInput, unionId string) (*model1.TerminalChange, error) {
	var rs model1.TerminalChange
...
func (r *queryResolver) TerminalChangeByUnionPk(ctx context.Context, unionId string) (*model1.TerminalChange, error) {
	var rs model1.TerminalChange
	tx := db.DB.Model(&model1.TerminalChange{}).Select(util.GetTopPreloads(ctx)).Where(rs.UnionPrimaryColumnName()+" = ?", unionId).First(&rs)
