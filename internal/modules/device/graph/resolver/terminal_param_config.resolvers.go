...
func (r *mutationResolver) DeleteTerminalParamConfig(ctx context.Context, where model.TerminalParamConfigBoolExp) (*model.TerminalParamConfigMutationResponse, error) {
	...
	var m = &model1.TerminalParamConfig{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteTerminalParamConfigByUnionPk(ctx context.Context, configID string) (*model1.TerminalParamConfig, error) {
	var rs model1.TerminalParamConfig
	m := &model1.TerminalParamConfig{}
	tx := db.DB.Model(&model1.TerminalParamConfig{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", configID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertTerminalParamConfigOne(ctx context.Context, objects model.TerminalParamConfigInsertInput) (*model1.TerminalParamConfig, error) {
...
func (r *mutationResolver) UpdateTerminalParamConfigByUnionPk(ctx context.Context, inc *model.TerminalParamConfigIncInput, set *model.TerminalParamConfigSetInput, configID string) (*model1.TerminalParamConfig, error) {
	var rs model1.TerminalParamConfig
	var m = &model1.TerminalParamConfig{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", configID)
	qt := util.NewQueryTranslator(tx, &model1.TerminalParamConfig{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) TerminalParamConfigByUnionPk(ctx context.Context, configID string) (*model1.TerminalParamConfig, error) {
	var rs model1.TerminalParamConfig
	var m = &model1.TerminalParamConfig{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", configID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
					_ = cacheAspect.SetNotExistUnionPkQueryCache(ctx, cacheKey, model1.TerminalParamConfig{})
				}
				return &rs, nil
			}
			return nil, err
