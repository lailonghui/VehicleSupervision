...
func (r *mutationResolver) DeleteTerminalBuyBillByUnionPk(ctx context.Context, billID string) (*model1.TerminalBuyBill, error) {
	var rs model1.TerminalBuyBill
	m := &model1.TerminalBuyBill{}
	tx := db.DB.Model(&model1.TerminalBuyBill{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", billID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertTerminalBuyBillOne(ctx context.Context, objects model.TerminalBuyBillInsertInput) (*model1.TerminalBuyBill, error) {
...
func (r *mutationResolver) UpdateTerminalBuyBillByUnionPk(ctx context.Context, inc *model.TerminalBuyBillIncInput, set *model.TerminalBuyBillSetInput, billID string) (*model1.TerminalBuyBill, error) {
	var rs model1.TerminalBuyBill
	var m = &model1.TerminalBuyBill{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", billID)
	qt := util.NewQueryTranslator(tx, &model1.TerminalBuyBill{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) TerminalBuyBillByUnionPk(ctx context.Context, billID string) (*model1.TerminalBuyBill, error) {
	var rs model1.TerminalBuyBill
	var m = &model1.TerminalBuyBill{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", billID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
					_ = cacheAspect.SetNotExistUnionPkQueryCache(ctx, cacheKey, model1.TerminalBuyBill{})
				}
				return &rs, nil
			}
			return nil, err
