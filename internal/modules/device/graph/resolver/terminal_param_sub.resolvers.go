...
func (r *mutationResolver) DeleteTerminalParamSubByUnionPk(ctx context.Context, paramSubID string) (*model1.TerminalParamSub, error) {
	var rs model1.TerminalParamSub
	m := &model1.TerminalParamSub{}
	tx := db.DB.Model(&model1.TerminalParamSub{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", paramSubID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertTerminalParamSubOne(ctx context.Context, objects model.TerminalParamSubInsertInput) (*model1.TerminalParamSub, error) {
...
func (r *mutationResolver) UpdateTerminalParamSubByUnionPk(ctx context.Context, inc *model.TerminalParamSubIncInput, set *model.TerminalParamSubSetInput, paramSubID string) (*model1.TerminalParamSub, error) {
	var rs model1.TerminalParamSub
	var m = &model1.TerminalParamSub{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", paramSubID)
	qt := util.NewQueryTranslator(tx, &model1.TerminalParamSub{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) TerminalParamSubByUnionPk(ctx context.Context, paramSubID string) (*model1.TerminalParamSub, error) {
	var rs model1.TerminalParamSub
	var m = &model1.TerminalParamSub{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", paramSubID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
					_ = cacheAspect.SetNotExistUnionPkQueryCache(ctx, cacheKey, model1.TerminalParamSub{})
				}
				return &rs, nil
			}
			return nil, err
