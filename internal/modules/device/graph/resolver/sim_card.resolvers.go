...
func (r *mutationResolver) DeleteSimCardByUnionPk(ctx context.Context, simCardID string) (*model1.SimCard, error) {
	var rs model1.SimCard
	m := &model1.SimCard{}
	tx := db.DB.Model(&model1.SimCard{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", simCardID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertSimCardOne(ctx context.Context, objects model.SimCardInsertInput) (*model1.SimCard, error) {
	rs := &model1.SimCard{}
	m := &model1.SimCard{}
	// 插入记录
...
func (r *mutationResolver) UpdateSimCardByUnionPk(ctx context.Context, inc *model.SimCardIncInput, set *model.SimCardSetInput, simCardID string) (*model1.SimCard, error) {
	var rs model1.SimCard
	var m = &model1.SimCard{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", simCardID)
	qt := util.NewQueryTranslator(tx, &model1.SimCard{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) SimCardByUnionPk(ctx context.Context, simCardID string) (*model1.SimCard, error) {
	var rs model1.SimCard
	var m = &model1.SimCard{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", simCardID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
					_ = cacheAspect.SetNotExistUnionPkQueryCache(ctx, cacheKey, model1.SimCard{})
				}
				return &rs, nil
			}
			return nil, err
