...
func (r *mutationResolver) DeleteTerminalCheckParam(ctx context.Context, where model.TerminalCheckParamBoolExp) (*model.TerminalCheckParamMutationResponse, error) {
	...
	var m = &model1.TerminalCheckParam{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteTerminalCheckParamByUnionPk(ctx context.Context, terminalCheckParamID string) (*model1.TerminalCheckParam, error) {
	var rs model1.TerminalCheckParam
	m := &model1.TerminalCheckParam{}
	tx := db.DB.Model(&model1.TerminalCheckParam{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", terminalCheckParamID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertTerminalCheckParamOne(ctx context.Context, objects model.TerminalCheckParamInsertInput) (*model1.TerminalCheckParam, error) {
...
func (r *mutationResolver) UpdateTerminalCheckParamByUnionPk(ctx context.Context, inc *model.TerminalCheckParamIncInput, set *model.TerminalCheckParamSetInput, terminalCheckParamID string) (*model1.TerminalCheckParam, error) {
	var rs model1.TerminalCheckParam
	var m = &model1.TerminalCheckParam{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", terminalCheckParamID)
	qt := util.NewQueryTranslator(tx, &model1.TerminalCheckParam{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) TerminalCheckParamByUnionPk(ctx context.Context, terminalCheckParamID string) (*model1.TerminalCheckParam, error) {
	var rs model1.TerminalCheckParam
	var m = &model1.TerminalCheckParam{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := terminalCheckParamID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", terminalCheckParamID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
					_ = cacheAspect.SetNotExistUnionPkQueryCache(ctx, cacheKey, model1.TerminalCheckParam{})
				}
				return &rs, nil
			}
			return nil, err
