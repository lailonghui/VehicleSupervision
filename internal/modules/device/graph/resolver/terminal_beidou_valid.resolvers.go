...
func (r *mutationResolver) DeleteTerminalBeidouValid(ctx context.Context, where model.TerminalBeidouValidBoolExp) (*model.TerminalBeidouValidMutationResponse, error) {
	...
	var m = &model1.TerminalBeidouValid{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteTerminalBeidouValidByUnionPk(ctx context.Context, terminalBeidouValidID string) (*model1.TerminalBeidouValid, error) {
	var rs model1.TerminalBeidouValid
	m := &model1.TerminalBeidouValid{}
	tx := db.DB.Model(&model1.TerminalBeidouValid{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", terminalBeidouValidID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertTerminalBeidouValidOne(ctx context.Context, objects model.TerminalBeidouValidInsertInput) (*model1.TerminalBeidouValid, error) {
...
func (r *mutationResolver) UpdateTerminalBeidouValidByUnionPk(ctx context.Context, inc *model.TerminalBeidouValidIncInput, set *model.TerminalBeidouValidSetInput, terminalBeidouValidID string) (*model1.TerminalBeidouValid, error) {
	var rs model1.TerminalBeidouValid
	var m = &model1.TerminalBeidouValid{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", terminalBeidouValidID)
	qt := util.NewQueryTranslator(tx, &model1.TerminalBeidouValid{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) TerminalBeidouValidByUnionPk(ctx context.Context, terminalBeidouValidID string) (*model1.TerminalBeidouValid, error) {
	var rs model1.TerminalBeidouValid
	var m = &model1.TerminalBeidouValid{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := terminalBeidouValidID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", terminalBeidouValidID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
