...
func (r *mutationResolver) DeleteTerminalProveByUnionPk(ctx context.Context, proveID string) (*model1.TerminalProve, error) {
	var rs model1.TerminalProve
	m := &model1.TerminalProve{}
	tx := db.DB.Model(&model1.TerminalProve{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", proveID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertTerminalProveOne(ctx context.Context, objects model.TerminalProveInsertInput) (*model1.TerminalProve, error) {
...
func (r *mutationResolver) UpdateTerminalProveByUnionPk(ctx context.Context, inc *model.TerminalProveIncInput, set *model.TerminalProveSetInput, proveID string) (*model1.TerminalProve, error) {
	var rs model1.TerminalProve
	var m = &model1.TerminalProve{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", proveID)
	qt := util.NewQueryTranslator(tx, &model1.TerminalProve{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) TerminalProveByUnionPk(ctx context.Context, proveID string) (*model1.TerminalProve, error) {
	var rs model1.TerminalProve
	var m = &model1.TerminalProve{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", proveID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
					_ = cacheAspect.SetNotExistUnionPkQueryCache(ctx, cacheKey, model1.TerminalProve{})
				}
				return &rs, nil
			}
			return nil, err
