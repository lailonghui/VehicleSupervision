...
func (r *mutationResolver) DeleteTerminalParamType(ctx context.Context, where model.TerminalParamTypeBoolExp) (*model.TerminalParamTypeMutationResponse, error) {
	...
	var m = &model1.TerminalParamType{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteTerminalParamTypeByUnionPk(ctx context.Context, paramTypeID string) (*model1.TerminalParamType, error) {
	var rs model1.TerminalParamType
	m := &model1.TerminalParamType{}
	tx := db.DB.Model(&model1.TerminalParamType{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", paramTypeID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertTerminalParamTypeOne(ctx context.Context, objects model.TerminalParamTypeInsertInput) (*model1.TerminalParamType, error) {
...
func (r *mutationResolver) UpdateTerminalParamTypeByUnionPk(ctx context.Context, inc *model.TerminalParamTypeIncInput, set *model.TerminalParamTypeSetInput, paramTypeID string) (*model1.TerminalParamType, error) {
	var rs model1.TerminalParamType
	var m = &model1.TerminalParamType{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", paramTypeID)
	qt := util.NewQueryTranslator(tx, &model1.TerminalParamType{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) TerminalParamTypeByUnionPk(ctx context.Context, paramTypeID string) (*model1.TerminalParamType, error) {
	var rs model1.TerminalParamType
	var m = &model1.TerminalParamType{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := paramTypeID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", paramTypeID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
					_ = cacheAspect.SetNotExistUnionPkQueryCache(ctx, cacheKey, model1.TerminalParamType{})
				}
				return &rs, nil
			}
			return nil, err
