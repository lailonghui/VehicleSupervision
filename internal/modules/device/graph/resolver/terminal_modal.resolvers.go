...
func (r *mutationResolver) DeleteTerminalModalByUnionPk(ctx context.Context, terminalModalID string) (*model1.TerminalModal, error) {
	var rs model1.TerminalModal
	m := &model1.TerminalModal{}
	tx := db.DB.Model(&model1.TerminalModal{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", terminalModalID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertTerminalModalOne(ctx context.Context, objects model.TerminalModalInsertInput) (*model1.TerminalModal, error) {
...
func (r *mutationResolver) UpdateTerminalModalByUnionPk(ctx context.Context, inc *model.TerminalModalIncInput, set *model.TerminalModalSetInput, terminalModalID string) (*model1.TerminalModal, error) {
	var rs model1.TerminalModal
	var m = &model1.TerminalModal{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", terminalModalID)
	qt := util.NewQueryTranslator(tx, &model1.TerminalModal{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) TerminalModalByUnionPk(ctx context.Context, terminalModalID string) (*model1.TerminalModal, error) {
	var rs model1.TerminalModal
	var m = &model1.TerminalModal{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := terminalModalID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", terminalModalID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
