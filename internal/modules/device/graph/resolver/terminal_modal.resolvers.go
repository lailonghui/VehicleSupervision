...
func (r *mutationResolver) DeleteTerminalModalByUnionPk(ctx context.Context, unionId string) (*model1.TerminalModal, error) {
	preloads := util.GetPreloads(ctx)
	var rs model1.TerminalModal
	tx := db.DB.Model(&model1.TerminalModal{})
	if len(preloads) > 0 {
...
func (r *mutationResolver) UpdateTerminalModalByUnionPk(ctx context.Context, inc *model.TerminalModalIncInput, set *model.TerminalModalSetInput, unionId string) (*model1.TerminalModal, error) {
	var rs model1.TerminalModal
...
func (r *queryResolver) TerminalModalByUnionPk(ctx context.Context, unionId string) (*model1.TerminalModal, error) {
	var rs model1.TerminalModal
	tx := db.DB.Model(&model1.TerminalModal{}).Select(util.GetTopPreloads(ctx)).Where(rs.UnionPrimaryColumnName()+" = ?", unionId).First(&rs)
