...
func (r *mutationResolver) DeleteTerminalFactoryByUnionPk(ctx context.Context, factoryID string) (*model1.TerminalFactory, error) {
	var rs model1.TerminalFactory
	m := &model1.TerminalFactory{}
	tx := db.DB.Model(&model1.TerminalFactory{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", factoryID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertTerminalFactoryOne(ctx context.Context, objects model.TerminalFactoryInsertInput) (*model1.TerminalFactory, error) {
...
func (r *mutationResolver) UpdateTerminalFactoryByUnionPk(ctx context.Context, inc *model.TerminalFactoryIncInput, set *model.TerminalFactorySetInput, factoryID string) (*model1.TerminalFactory, error) {
	var rs model1.TerminalFactory
	var m = &model1.TerminalFactory{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", factoryID)
	qt := util.NewQueryTranslator(tx, &model1.TerminalFactory{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) TerminalFactoryByUnionPk(ctx context.Context, factoryID string) (*model1.TerminalFactory, error) {
	var rs model1.TerminalFactory
	var m = &model1.TerminalFactory{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", factoryID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
