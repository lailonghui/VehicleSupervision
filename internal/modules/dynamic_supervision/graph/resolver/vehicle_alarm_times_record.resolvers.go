...
func (r *mutationResolver) DeleteVehicleAlarmTimesRecord(ctx context.Context, where model.VehicleAlarmTimesRecordBoolExp) (*model.VehicleAlarmTimesRecordMutationResponse, error) {
	...
	var m = &model1.VehicleAlarmTimesRecord{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteVehicleAlarmTimesRecordByPk(ctx context.Context, id int64) (*model1.VehicleAlarmTimesRecord, error) {
	var rs model1.VehicleAlarmTimesRecord
	m := &model1.VehicleAlarmTimesRecord{}
	tx := db.DB.Model(m)
	// 查询记录
...
func (r *mutationResolver) DeleteVehicleAlarmTimesRecordByUnionPk(ctx context.Context, vehicleAlarmTimesRecordID string) (*model1.VehicleAlarmTimesRecord, error) {
	var rs model1.VehicleAlarmTimesRecord
	m := &model1.VehicleAlarmTimesRecord{}
	tx := db.DB.Model(&model1.VehicleAlarmTimesRecord{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", vehicleAlarmTimesRecordID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertVehicleAlarmTimesRecord(ctx context.Context, objects []*model.VehicleAlarmTimesRecordInsertInput) (*model.VehicleAlarmTimesRecordMutationResponse, error) {
	rs := make([]*model1.VehicleAlarmTimesRecord, 0)
	m := &model1.VehicleAlarmTimesRecord{}
	// 结构转换
...
func (r *mutationResolver) InsertVehicleAlarmTimesRecordOne(ctx context.Context, objects model.VehicleAlarmTimesRecordInsertInput) (*model1.VehicleAlarmTimesRecord, error) {
...
func (r *mutationResolver) UpdateVehicleAlarmTimesRecordByUnionPk(ctx context.Context, inc *model.VehicleAlarmTimesRecordIncInput, set *model.VehicleAlarmTimesRecordSetInput, vehicleAlarmTimesRecordID string) (*model1.VehicleAlarmTimesRecord, error) {
	var rs model1.VehicleAlarmTimesRecord
	var m = &model1.VehicleAlarmTimesRecord{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", vehicleAlarmTimesRecordID)
	qt := util.NewQueryTranslator(tx, &model1.VehicleAlarmTimesRecord{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) VehicleAlarmTimesRecordByUnionPk(ctx context.Context, vehicleAlarmTimesRecordID string) (*model1.VehicleAlarmTimesRecord, error) {
	var rs model1.VehicleAlarmTimesRecord
	var m = &model1.VehicleAlarmTimesRecord{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := vehicleAlarmTimesRecordID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", vehicleAlarmTimesRecordID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
