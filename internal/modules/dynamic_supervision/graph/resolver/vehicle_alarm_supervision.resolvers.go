...
func (r *mutationResolver) DeleteVehicleAlarmSupervision(ctx context.Context, where model.VehicleAlarmSupervisionBoolExp) (*model.VehicleAlarmSupervisionMutationResponse, error) {
	...
	var m = &model1.VehicleAlarmSupervision{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteVehicleAlarmSupervisionByPk(ctx context.Context, id int64) (*model1.VehicleAlarmSupervision, error) {
	var rs model1.VehicleAlarmSupervision
	m := &model1.VehicleAlarmSupervision{}
	tx := db.DB.Model(m)
	// 查询记录
...
func (r *mutationResolver) DeleteVehicleAlarmSupervisionByUnionPk(ctx context.Context, vehicleAlarmSupervisionID string) (*model1.VehicleAlarmSupervision, error) {
	var rs model1.VehicleAlarmSupervision
	m := &model1.VehicleAlarmSupervision{}
	tx := db.DB.Model(&model1.VehicleAlarmSupervision{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", vehicleAlarmSupervisionID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertVehicleAlarmSupervision(ctx context.Context, objects []*model.VehicleAlarmSupervisionInsertInput) (*model.VehicleAlarmSupervisionMutationResponse, error) {
	rs := make([]*model1.VehicleAlarmSupervision, 0)
	m := &model1.VehicleAlarmSupervision{}
	// 结构转换
...
func (r *mutationResolver) InsertVehicleAlarmSupervisionOne(ctx context.Context, objects model.VehicleAlarmSupervisionInsertInput) (*model1.VehicleAlarmSupervision, error) {
...
func (r *mutationResolver) UpdateVehicleAlarmSupervisionByUnionPk(ctx context.Context, inc *model.VehicleAlarmSupervisionIncInput, set *model.VehicleAlarmSupervisionSetInput, vehicleAlarmSupervisionID string) (*model1.VehicleAlarmSupervision, error) {
	var rs model1.VehicleAlarmSupervision
	var m = &model1.VehicleAlarmSupervision{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", vehicleAlarmSupervisionID)
	qt := util.NewQueryTranslator(tx, &model1.VehicleAlarmSupervision{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) VehicleAlarmSupervisionByUnionPk(ctx context.Context, vehicleAlarmSupervisionID string) (*model1.VehicleAlarmSupervision, error) {
	var rs model1.VehicleAlarmSupervision
	var m = &model1.VehicleAlarmSupervision{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := vehicleAlarmSupervisionID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", vehicleAlarmSupervisionID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
