...
func (r *mutationResolver) DeleteRegionIssuedByUnionPk(ctx context.Context, regionIssuedID string) (*model1.RegionIssued, error) {
	var rs model1.RegionIssued
	m := &model1.RegionIssued{}
	tx := db.DB.Model(&model1.RegionIssued{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", regionIssuedID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertRegionIssuedOne(ctx context.Context, objects model.RegionIssuedInsertInput) (*model1.RegionIssued, error) {
...
func (r *mutationResolver) UpdateRegionIssuedByUnionPk(ctx context.Context, inc *model.RegionIssuedIncInput, set *model.RegionIssuedSetInput, regionIssuedID string) (*model1.RegionIssued, error) {
	var rs model1.RegionIssued
	var m = &model1.RegionIssued{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", regionIssuedID)
	qt := util.NewQueryTranslator(tx, &model1.RegionIssued{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) RegionIssuedByUnionPk(ctx context.Context, regionIssuedID string) (*model1.RegionIssued, error) {
	var rs model1.RegionIssued
	var m = &model1.RegionIssued{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := regionIssuedID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", regionIssuedID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
