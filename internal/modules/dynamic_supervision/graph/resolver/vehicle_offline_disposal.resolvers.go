...
func (r *mutationResolver) DeleteVehicleOfflineDisposal(ctx context.Context, where model.VehicleOfflineDisposalBoolExp) (*model.VehicleOfflineDisposalMutationResponse, error) {
	...
	var m = &model1.VehicleOfflineDisposal{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteVehicleOfflineDisposalByPk(ctx context.Context, id int64) (*model1.VehicleOfflineDisposal, error) {
	var rs model1.VehicleOfflineDisposal
	m := &model1.VehicleOfflineDisposal{}
	tx := db.DB.Model(m)
	// 查询记录
...
func (r *mutationResolver) DeleteVehicleOfflineDisposalByUnionPk(ctx context.Context, vehicleOfflineDisposalID string) (*model1.VehicleOfflineDisposal, error) {
	var rs model1.VehicleOfflineDisposal
	m := &model1.VehicleOfflineDisposal{}
	tx := db.DB.Model(&model1.VehicleOfflineDisposal{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", vehicleOfflineDisposalID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertVehicleOfflineDisposalOne(ctx context.Context, objects model.VehicleOfflineDisposalInsertInput) (*model1.VehicleOfflineDisposal, error) {
...
func (r *mutationResolver) UpdateVehicleOfflineDisposalByUnionPk(ctx context.Context, inc *model.VehicleOfflineDisposalIncInput, set *model.VehicleOfflineDisposalSetInput, vehicleOfflineDisposalID string) (*model1.VehicleOfflineDisposal, error) {
	var rs model1.VehicleOfflineDisposal
	var m = &model1.VehicleOfflineDisposal{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", vehicleOfflineDisposalID)
	qt := util.NewQueryTranslator(tx, &model1.VehicleOfflineDisposal{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) VehicleOfflineDisposalByUnionPk(ctx context.Context, vehicleOfflineDisposalID string) (*model1.VehicleOfflineDisposal, error) {
	var rs model1.VehicleOfflineDisposal
	var m = &model1.VehicleOfflineDisposal{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := vehicleOfflineDisposalID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", vehicleOfflineDisposalID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
