...
func (r *mutationResolver) DeleteDynamicSupervision(ctx context.Context, where model.DynamicSupervisionBoolExp) (*model.DynamicSupervisionMutationResponse, error) {
	...
	var m = &model1.DynamicSupervision{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteDynamicSupervisionByUnionPk(ctx context.Context, supervisionID string) (*model1.DynamicSupervision, error) {
	var rs model1.DynamicSupervision
	m := &model1.DynamicSupervision{}
	tx := db.DB.Model(&model1.DynamicSupervision{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", supervisionID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertDynamicSupervisionOne(ctx context.Context, objects model.DynamicSupervisionInsertInput) (*model1.DynamicSupervision, error) {
...
func (r *mutationResolver) UpdateDynamicSupervisionByUnionPk(ctx context.Context, inc *model.DynamicSupervisionIncInput, set *model.DynamicSupervisionSetInput, supervisionID string) (*model1.DynamicSupervision, error) {
	var rs model1.DynamicSupervision
	var m = &model1.DynamicSupervision{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", supervisionID)
	qt := util.NewQueryTranslator(tx, &model1.DynamicSupervision{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) DynamicSupervisionByUnionPk(ctx context.Context, supervisionID string) (*model1.DynamicSupervision, error) {
	var rs model1.DynamicSupervision
	var m = &model1.DynamicSupervision{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := supervisionID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", supervisionID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
