...
func (r *mutationResolver) DeleteRegionManagementByUnionPk(ctx context.Context, regionManagementID string) (*model1.RegionManagement, error) {
	var rs model1.RegionManagement
	m := &model1.RegionManagement{}
	tx := db.DB.Model(&model1.RegionManagement{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", regionManagementID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertRegionManagementOne(ctx context.Context, objects model.RegionManagementInsertInput) (*model1.RegionManagement, error) {
...
func (r *mutationResolver) UpdateRegionManagementByUnionPk(ctx context.Context, inc *model.RegionManagementIncInput, set *model.RegionManagementSetInput, regionManagementID string) (*model1.RegionManagement, error) {
	var rs model1.RegionManagement
	var m = &model1.RegionManagement{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", regionManagementID)
	qt := util.NewQueryTranslator(tx, &model1.RegionManagement{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) RegionManagementByUnionPk(ctx context.Context, regionManagementID string) (*model1.RegionManagement, error) {
	var rs model1.RegionManagement
	var m = &model1.RegionManagement{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := regionManagementID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", regionManagementID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
