...
func (r *mutationResolver) DeleteMuckTruckOnlineByUnionPk(ctx context.Context, muckTruckOnlineID string) (*model1.MuckTruckOnline, error) {
	var rs model1.MuckTruckOnline
	m := &model1.MuckTruckOnline{}
	tx := db.DB.Model(&model1.MuckTruckOnline{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", muckTruckOnlineID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertMuckTruckOnline(ctx context.Context, objects []*model.MuckTruckOnlineInsertInput) (*model.MuckTruckOnlineMutationResponse, error) {
	rs := make([]*model1.MuckTruckOnline, 0)
	m := &model1.MuckTruckOnline{}
	// 结构转换
...
func (r *mutationResolver) InsertMuckTruckOnlineOne(ctx context.Context, objects model.MuckTruckOnlineInsertInput) (*model1.MuckTruckOnline, error) {
...
func (r *mutationResolver) UpdateMuckTruckOnlineByUnionPk(ctx context.Context, inc *model.MuckTruckOnlineIncInput, set *model.MuckTruckOnlineSetInput, muckTruckOnlineID string) (*model1.MuckTruckOnline, error) {
	var rs model1.MuckTruckOnline
	var m = &model1.MuckTruckOnline{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", muckTruckOnlineID)
	qt := util.NewQueryTranslator(tx, &model1.MuckTruckOnline{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) MuckTruckOnlineByUnionPk(ctx context.Context, muckTruckOnlineID string) (*model1.MuckTruckOnline, error) {
	var rs model1.MuckTruckOnline
	var m = &model1.MuckTruckOnline{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		cacheAspect, cacheErr := cache.GetGqlCacheAspect(rs.TableName())
		cacheKey := muckTruckOnlineID
		if cacheErr == nil {
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", muckTruckOnlineID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
