...
// 新建主键dataloader
func (t *EnterpriseBlacklistAlarmPkLoader) NewLoader(ctx context.Context) *EnterpriseBlacklistAlarmPkLoader {
	return &EnterpriseBlacklistAlarmPkLoader{
		wait:     2 * time.Millisecond,
		maxBatch: 100,
		fetch: func(keys []string) ([]*EnterpriseBlacklistAlarm, []error) {
			var rs []*EnterpriseBlacklistAlarm = make([]*EnterpriseBlacklistAlarm, len(keys))
			...
			if middle.IsEnableGqlCache(ctx) {
				...
				for i := range keys {
					...
					go func(i int) {
						...
						// 如果启用缓存，则从缓存中查询数据
						var entity EnterpriseBlacklistAlarm
						cacheKey := keys[i]
						if cacheErr == nil {
...
// 新建联合主键dataloader
func (t *EnterpriseBlacklistAlarmUnionPkLoader) NewLoader(ctx context.Context) *EnterpriseBlacklistAlarmUnionPkLoader {
