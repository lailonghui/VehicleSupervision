...
// 新建主键dataloader
func (t *VehicleBlacklistAlarmPkLoader) NewLoader(ctx context.Context) *VehicleBlacklistAlarmPkLoader {
	return &VehicleBlacklistAlarmPkLoader{
		wait:     2 * time.Millisecond,
		maxBatch: 100,
		fetch: func(keys []string) ([]*VehicleBlacklistAlarm, []error) {
			var rs []*VehicleBlacklistAlarm = make([]*VehicleBlacklistAlarm, len(keys))
			...
			if middle.IsEnableGqlCache(ctx) {
				...
				for i := range keys {
					...
					go func(i int) {
						...
						// 如果启用缓存，则从缓存中查询数据
						var entity VehicleBlacklistAlarm
						cacheKey := keys[i]
						if cacheErr == nil {
...
// 新建联合主键dataloader
func (t *VehicleBlacklistAlarmUnionPkLoader) NewLoader(ctx context.Context) *VehicleBlacklistAlarmUnionPkLoader {
