...
// 新建主键dataloader
func (t *DriverBlacklistApplyPkLoader) NewLoader(ctx context.Context) *DriverBlacklistApplyPkLoader {
	return &DriverBlacklistApplyPkLoader{
		wait:     2 * time.Millisecond,
		maxBatch: 100,
		fetch: func(keys []string) ([]*DriverBlacklistApply, []error) {
			var rs []*DriverBlacklistApply = make([]*DriverBlacklistApply, len(keys))
			...
			if middle.IsEnableGqlCache(ctx) {
				...
				for i := range keys {
					...
					go func(i int) {
						...
						// 如果启用缓存，则从缓存中查询数据
						var entity DriverBlacklistApply
						cacheKey := keys[i]
						if cacheErr == nil {
...
// 新建联合主键dataloader
func (t *DriverBlacklistApplyUnionPkLoader) NewLoader(ctx context.Context) *DriverBlacklistApplyUnionPkLoader {
