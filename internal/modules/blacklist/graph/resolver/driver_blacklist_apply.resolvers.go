...
func (r *mutationResolver) DeleteDriverBlacklistApply(ctx context.Context, where model.DriverBlacklistApplyBoolExp) (*model.DriverBlacklistApplyMutationResponse, error) {
	...
	var m = &model1.DriverBlacklistApply{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteDriverBlacklistApplyByUnionPk(ctx context.Context, applyID string) (*model1.DriverBlacklistApply, error) {
	var rs model1.DriverBlacklistApply
	m := &model1.DriverBlacklistApply{}
	tx := db.DB.Model(&model1.DriverBlacklistApply{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", applyID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertDriverBlacklistApplyOne(ctx context.Context, objects model.DriverBlacklistApplyInsertInput) (*model1.DriverBlacklistApply, error) {
...
func (r *mutationResolver) UpdateDriverBlacklistApplyByUnionPk(ctx context.Context, inc *model.DriverBlacklistApplyIncInput, set *model.DriverBlacklistApplySetInput, applyID string) (*model1.DriverBlacklistApply, error) {
	var rs model1.DriverBlacklistApply
	var m = &model1.DriverBlacklistApply{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", applyID)
	qt := util.NewQueryTranslator(tx, &model1.DriverBlacklistApply{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) DriverBlacklistApplyByUnionPk(ctx context.Context, applyID string) (*model1.DriverBlacklistApply, error) {
	var rs model1.DriverBlacklistApply
	var m = &model1.DriverBlacklistApply{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", applyID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
