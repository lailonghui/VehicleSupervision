...
func (r *mutationResolver) DeleteEnterpriseBlacklistAlarm(ctx context.Context, where model.EnterpriseBlacklistAlarmBoolExp) (*model.EnterpriseBlacklistAlarmMutationResponse, error) {
	...
	var m = &model1.EnterpriseBlacklistAlarm{}
	// 查询主键和联合主键
...
func (r *mutationResolver) DeleteEnterpriseBlacklistAlarmByPk(ctx context.Context, id int64) (*model1.EnterpriseBlacklistAlarm, error) {
	var rs model1.EnterpriseBlacklistAlarm
	m := &model1.EnterpriseBlacklistAlarm{}
	tx := db.DB.Model(m)
	// 查询记录
...
func (r *mutationResolver) DeleteEnterpriseBlacklistAlarmByUnionPk(ctx context.Context, alarmID string) (*model1.EnterpriseBlacklistAlarm, error) {
	var rs model1.EnterpriseBlacklistAlarm
	m := &model1.EnterpriseBlacklistAlarm{}
	tx := db.DB.Model(&model1.EnterpriseBlacklistAlarm{})
	// 查询
	tx = tx.Where(rs.UnionPrimaryColumnName()+" = ?", alarmID).First(&rs)
	if err := tx.Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *mutationResolver) InsertEnterpriseBlacklistAlarmOne(ctx context.Context, objects model.EnterpriseBlacklistAlarmInsertInput) (*model1.EnterpriseBlacklistAlarm, error) {
...
func (r *mutationResolver) UpdateEnterpriseBlacklistAlarmByUnionPk(ctx context.Context, inc *model.EnterpriseBlacklistAlarmIncInput, set *model.EnterpriseBlacklistAlarmSetInput, alarmID string) (*model1.EnterpriseBlacklistAlarm, error) {
	var rs model1.EnterpriseBlacklistAlarm
	var m = &model1.EnterpriseBlacklistAlarm{}
	// 更新数据库
	tx := db.DB.Where(rs.UnionPrimaryColumnName()+" = ?", alarmID)
	qt := util.NewQueryTranslator(tx, &model1.EnterpriseBlacklistAlarm{})
	tx = qt.Inc(inc).Set(set).DoUpdate()
	if err := tx.Error; err != nil {
		return nil, err
...
func (r *queryResolver) EnterpriseBlacklistAlarmByUnionPk(ctx context.Context, alarmID string) (*model1.EnterpriseBlacklistAlarm, error) {
	var rs model1.EnterpriseBlacklistAlarm
	var m = &model1.EnterpriseBlacklistAlarm{}
	if middle.IsEnableGqlCache(ctx) {
		// 如果启用缓存，则从缓存中查询数据
		...
		// 缓存中找不到数据的话，查询数据库获取数据
		tx := db.DB.Model(m).Where(rs.UnionPrimaryColumnName()+" = ?", alarmID).First(&rs)
		err := tx.Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				if cacheErr == nil {
					_ = cacheAspect.SetNotExistUnionPkQueryCache(ctx, cacheKey, model1.EnterpriseBlacklistAlarm{})
				}
				return &rs, nil
			}
			return nil, err
